...
async function loadData() {
  ...
  // PLANILHA_DATA fallback removido - dados vêm apenas do servidor/localStorage
...
function restaurarPlanilha() {
  alert('Função desativada. Os dados agora são gerenciados exclusivamente pelo servidor.');
}
...
async function salvarSocio() {
  ...
  if (!cpf && !rg) { showMsg('err', '\u274C Informe ao menos CPF ou RG.'); return; }
...
async function carregarSocios() {
  try {
    ...
    if (response.ok) {
      ...
      // Merge com PLANILHA_DATA removido - dados vêm apenas do servidor
      try { localStorage.setItem(LOCAL_KEY, JSON.stringify(socios)); } catch(e) {}
      updateHeader();
      renderSocios();
      renderTitulos();
    ...
    // Servidor indisponível - usa apenas dados locais (localStorage)
    updateHeader();
    renderSocios();
    renderTitulos();
    renderDashboard();
