...
function renderSocios(resetPag) {
  ...
  const paginacaoHtml=totalPags>1?`<div style="display:flex;align-items:center;gap:8px;margin:8px 0 4px;flex-wrap:wrap">
    ${_sociosPag>1?'<button type="button" class="btn-outline" style="padding:4px 10px;font-size:12px" onclick="window.renderSocios._pag(-1)">← Anterior</button>':''}
    <span style="font-size:12px;color:#6b7280;padding:0 8px">P\u00e1g ${_sociosPag} de ${totalPags} &middot; ${total} s\u00f3cios</span>
    ${_sociosPag<totalPags?'<button type="button" class="btn-outline" style="padding:4px 10px;font-size:12px" onclick="window.renderSocios._pag(1)">Pr\u00f3xima \u2192</button>':''}
  </div>`:'';
...
renderSocios._pag=function(dir){
  const total = (typeof socios!=='undefined'?socios.length:0);
