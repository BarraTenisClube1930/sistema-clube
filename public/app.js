...
function renderDashboard(){
  ...
  const irregulares=socios.filter(s=>getSitNorm(s)==='irregular').length;
  ...
  if(execEl){
    ...
    execEl.innerHTML=`
      <p>O clube possui atualmente <strong>${total} sócios cadastrados</strong>, dos quais <strong>${ativos} estão ativos (${pctAtivos}%)</strong>${suspensos?`, ${suspensos} suspensos`:''}${desligados?`, ${desligados} desligados`:''}${inadimplentes?`, ${inadimplentes} inadimplentes (${pctInad}%)`:''}${irregulares?` e ${irregulares} irregulares (sem documentação completa)`:''}.</p>
  ...
  if(alertasEl){
    ...
    if(irregulares>0) alertas.push({tipo:'critico',icon:'📋',titulo:'Documentação Irregular',texto:`${irregulares} sócio(s) com situação Irregular — sem documentação completa (apenas ficha histórica). Não geram carteirinha.`});
