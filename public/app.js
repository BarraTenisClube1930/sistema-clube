...
// ===== DEPENDENTES =====
// Sanitiza input type="date": rejeita anos inválidos (ex.: "0002") típicos de digitação parcial.
function _sanitizeDataNasc(v, inputEl){
  if(!v) return '';
  const m = /^(\d{1,4})-(\d{2})-(\d{2})$/.exec(v);
  if(!m){ if(inputEl) inputEl.value=''; return ''; }
  const ano = parseInt(m[1],10);
  const anoAtual = new Date().getFullYear();
  if(ano < 1900 || ano > anoAtual){
    if(inputEl){ inputEl.value=''; inputEl.setCustomValidity('Ano inválido — informe entre 1900 e '+anoAtual); inputEl.reportValidity && inputEl.reportValidity(); setTimeout(()=>inputEl.setCustomValidity(''),50); }
    return '';
  }
  return v;
}
function addDep(){const i=depCounter++;depsData.push({id:i,nome:'',dataNasc:'',sexo:'M',tipoDep:'filho',foto:'',inicioUniao:'',estadoCivilDep:'solteira',incapaz:false,dependenciaFinanceira:false,obsAuditoria:'',docs:{registroCivil:false,tutela:false,declaracao2Socios:false}});renderDeps();}
function removeDep(id){depsData=depsData.filter(d=>d.id!==id);renderDeps();}
function _depTipoOptions(sel){
  const grupos = [
    {label:'Previstos no Art. 22 (regulares se requisitos atendidos)', opts:[
      ['filho','Filho(a) menor de 18 anos'],
      ['filha_solteira','Filha solteira (qualquer idade)'],
      ['enteado','Enteado(a)'],
      ['tutelado','Tutelado(a) / Guarda judicial'],
      ['conjuge','Cônjuge'],
      ['companheiro','Companheiro(a) - união estável >2 anos'],
      ['pai_mae_60','Pai/Mãe (≥60 anos OU dep. financeira)'],
      ['sogro','Sogro(a)'],
      ['padrasto_madrasta','Padrasto/Madrasta'],
      ['irmao_menor','Irmão(ã) menor de 18 anos na admissão'],
    ]},
    {label:'NÃO previstos no Art. 22 (sempre irregulares)', opts:[
      ['neto','Neto(a)'],
      ['sobrinho','Sobrinho(a)'],
      ['primo','Primo(a)'],
      ['outro_irregular','Outro - fora do Art. 22'],
    ]}
  ];
  return grupos.map(g => `<optgroup label="${g.label}">` +
    g.opts.map(([v,l]) => `<option value="${v}" ${sel===v?'selected':''}>${l}</option>`).join('') +
    `</optgroup>`).join('');
}
function _depCamposCondicionais(dep, prefix){
  // prefix: 'depsData' ou 'editDepsData'
  const i = dep._idx ?? dep.id;
  const idxAccess = prefix==='depsData'
    ? `${prefix}.find(d=>d.id===${dep.id})`
    : `${prefix}[${i}]`;
  let html = '';
  if (dep.tipoDep === 'companheiro') {
    html += `<div class="field"><label>Início da união estável</label><input type="date" value="${dep.inicioUniao||''}" onchange="${idxAccess}.inicioUniao=this.value;_rerenderDepCard('${prefix}',${dep.id ?? i})"/></div>`;
  }
  if (['filho','enteado','tutelado'].includes(dep.tipoDep) && dep.sexo === 'M') {
    html += `<div class="field col2"><label style="display:flex;align-items:center;gap:6px;cursor:pointer"><input type="checkbox" ${dep.incapaz?'checked':''} onchange="${idxAccess}.incapaz=this.checked;_rerenderDepCard('${prefix}',${dep.id ?? i})"/> Incapaz física/mentalmente (laudo anexado) — Art. 22-c</label></div>`;
  }
  if (['filha_solteira','filho','enteado','tutelado'].includes(dep.tipoDep) && dep.sexo === 'F') {
    html += `<div class="field"><label>Estado civil da dependente</label><select onchange="${idxAccess}.estadoCivilDep=this.value;_rerenderDepCard('${prefix}',${dep.id ?? i})">
      <option value="solteira" ${dep.estadoCivilDep==='solteira'?'selected':''}>Solteira</option>
      <option value="casada" ${dep.estadoCivilDep==='casada'?'selected':''}>Casada</option>
      <option value="divorciada" ${dep.estadoCivilDep==='divorciada'?'selected':''}>Divorciada</option>
      <option value="viuva" ${dep.estadoCivilDep==='viuva'?'selected':''}>Viúva</option>
      <option value="uniao_estavel" ${dep.estadoCivilDep==='uniao_estavel'?'selected':''}>União estável</option>
    </select></div>`;
  }
  if (['pai_mae_60','sogro','padrasto_madrasta'].includes(dep.tipoDep)) {
    html += `<div class="field col2"><label style="display:flex;align-items:center;gap:6px;cursor:pointer"><input type="checkbox" ${dep.dependenciaFinanceira?'checked':''} onchange="${idxAccess}.dependenciaFinanceira=this.checked;_rerenderDepCard('${prefix}',${dep.id ?? i})"/> Dependência financeira comprovada — Art. 22-f</label></div>`;
  }
  // Documentos comprobatórios (Art. 22-e)
  const docs = dep.docs || {};
  html += `<div class="field col2" style="background:#f8fafc;padding:8px;border-radius:6px;border:1px solid #e2e8f0">
    <label style="font-size:11px;font-weight:700;color:#475569;display:block;margin-bottom:4px">📄 Documentos comprobatórios (Art. 22-e)</label>
    <div style="display:flex;flex-wrap:wrap;gap:10px;font-size:12px">
      <label style="display:flex;align-items:center;gap:4px;cursor:pointer"><input type="checkbox" ${docs.registroCivil?'checked':''} onchange="${idxAccess}.docs=Object.assign(${idxAccess}.docs||{},{registroCivil:this.checked});_rerenderDepCard('${prefix}',${dep.id ?? i})"/> Registro Civil</label>
      <label style="display:flex;align-items:center;gap:4px;cursor:pointer"><input type="checkbox" ${docs.tutela?'checked':''} onchange="${idxAccess}.docs=Object.assign(${idxAccess}.docs||{},{tutela:this.checked});_rerenderDepCard('${prefix}',${dep.id ?? i})"/> Termo de tutela/guarda</label>
      <label style="display:flex;align-items:center;gap:4px;cursor:pointer"><input type="checkbox" ${docs.declaracao2Socios?'checked':''} onchange="${idxAccess}.docs=Object.assign(${idxAccess}.docs||{},{declaracao2Socios:this.checked});_rerenderDepCard('${prefix}',${dep.id ?? i})"/> Declaração 2 sócios</label>
    </div>
  </div>`;
  html += `<div class="field col2"><label>Observações da auditoria</label><input type="text" placeholder="Ex: dependente cadastrado sem comprovação em 2015" value="${dep.obsAuditoria||''}" oninput="${idxAccess}.obsAuditoria=this.value"/></div>`;
  return html;
}
function _rerenderDepCard(prefix){
  if (prefix==='depsData') renderDeps();
  else _renderEditDepsList();
}
function renderDeps(){
  const list=document.getElementById('deps-list');
  if(!depsData.length){list.innerHTML='';return;}
  const _pseudoSocio = { dataAcordo: document.getElementById('f-dataacordo')?.value || '' };
  list.innerHTML=depsData.map((dep,i)=>{
    const av = avaliarDependente(dep, _pseudoSocio);
    const motivosHtml = av.motivos.length ? `<div style="font-size:11px;color:#7f1d1d;background:#fef2f2;padding:6px 8px;border-radius:4px;margin-top:6px;border-left:3px solid #dc2626">${av.motivos.map(m=>'• '+sanitize(m)).join('<br/>')}</div>` : '';
  return `
    <div class="dep-card">
      <div class="dep-header" style="display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap">
        <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap"><strong>Dependente #${i+1}</strong>${statusDepBadge(av.status)}</div>
        <button type="button" class="btn-del" onclick="removeDep(${dep.id})">Remover</button>
      </div>
      ${motivosHtml}
      <div class="grid2" style="margin-top:8px">
        <div class="field col2"><label>Nome Completo</label><input type="text" value="${dep.nome}" oninput="depsData.find(d=>d.id===${dep.id}).nome=this.value"/></div>
        <div class="field col2"><label>Tipo de Dependente (Art. 22)</label>
          <select onchange="depsData.find(d=>d.id===${dep.id}).tipoDep=this.value;renderDeps()">
            ${_depTipoOptions(dep.tipoDep)}
          </select>
        </div>
        <div class="field"><label>Data de Nascimento</label><input type="date" min="1900-01-01" max="${new Date().toISOString().split('T')[0]}" value="${dep.dataNasc}" onchange="depsData.find(d=>d.id===${dep.id}).dataNasc=_sanitizeDataNasc(this.value,this);renderDeps()"/></div>
        <div class="field"><label>Sexo</label>
          <select onchange="depsData.find(d=>d.id===${dep.id}).sexo=this.value;renderDeps()">
            <option value="M" ${dep.sexo==='M'?'selected':''}>Masculino</option>
            <option value="F" ${dep.sexo==='F'?'selected':''}>Feminino</option>
          </select>
        </div>
        ${_depCamposCondicionais(dep,'depsData')}
      </div>
...
function renderEditDeps(s) {
  if (!s) return;
  editDepsData = (s.dependentes||[]).map((d,i) => ({
    nome:'', dataNasc:'', sexo:'M', tipoDep:'filho', foto:'',
    inicioUniao:'', estadoCivilDep:'solteira', incapaz:false,
    dependenciaFinanceira:false, obsAuditoria:'',
    docs:{registroCivil:false,tutela:false,declaracao2Socios:false},
    ...d, _idx: i
  }));
...
function _renderEditDepsList() {
  ...
  const _liveAcordo = document.getElementById('e-dataacordo')?.value || (_editDepSocio && _editDepSocio.dataAcordo) || '';
  const _liveSocio = Object.assign({}, _editDepSocio || {}, { dataAcordo: _liveAcordo });
  list.innerHTML = editDepsData.map((dep, i) => {
    dep._idx = i;
    const av = avaliarDependente(dep, _liveSocio);
    const motivosHtml = av.motivos.length ? `<div style="font-size:11px;color:#7f1d1d;background:#fef2f2;padding:6px 8px;border-radius:4px;margin-top:6px;border-left:3px solid #dc2626">${av.motivos.map(m=>'• '+sanitize(m)).join('<br/>')}</div>` : '';
    return `
    <div class="dep-card" style="margin-bottom:10px">
      <div class="dep-header" style="display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap">
        <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap"><strong>Dependente #${i+1}</strong>${statusDepBadge(av.status)}</div>
        <button type="button" class="btn-del" onclick="removeEditDep(${i})">Remover</button>
      </div>
      ${motivosHtml}
      <div class="grid2" style="margin-top:8px">
        <div class="field col2"><label>Nome Completo</label><input type="text" value="${dep.nome||''}" oninput="editDepsData[${i}].nome=this.value"/></div>
        <div class="field col2"><label>Tipo de Dependente (Art. 22)</label>
          <select onchange="editDepsData[${i}].tipoDep=this.value;_renderEditDepsList()">
            ${_depTipoOptions(dep.tipoDep)}
          </select>
        </div>
        <div class="field"><label>Data de Nascimento</label><input type="date" min="1900-01-01" max="${new Date().toISOString().split('T')[0]}" value="${dep.dataNasc||''}" onchange="editDepsData[${i}].dataNasc=_sanitizeDataNasc(this.value,this);_renderEditDepsList()"/></div>
        <div class="field"><label>Sexo</label>
          <select onchange="editDepsData[${i}].sexo=this.value;_renderEditDepsList()">
            <option value="M" ${dep.sexo==='M'?'selected':''}>Masculino</option>
            <option value="F" ${dep.sexo==='F'?'selected':''}>Feminino</option>
          </select>
        </div>
        ${_depCamposCondicionais(dep,'editDepsData')}
      </div>
    </div>`}).join('');
}
...
async function salvarSocio() {
  ...
  const socio = {
    ...
    motivoIrregular:  (document.getElementById('f-motivo-irregular')?.value||'').trim(),
    ...
    presidente:       canonPresidente((document.getElementById('f-presidente')?.value||'').trim()),
    dataAcordo:       (document.getElementById('f-dataacordo-ni')?.checked ? '' : (document.getElementById('f-dataacordo')?.value || '')),
  ...
  // Validação: situação Irregular exige motivo
  if (socio.situacao === 'irregular' && !socio.motivoIrregular) {
    showMsg('err', '⚠️ Quando a situação é "Irregular", é obrigatório informar o Motivo da Irregularidade.');
    document.getElementById('f-motivo-irregular')?.focus();
    return;
  }
...
function resetForm(){
  ...
  if(document.getElementById('f-motivo-irregular'))document.getElementById('f-motivo-irregular').value='';
  if(document.getElementById('f-motivo-irregular-wrap'))document.getElementById('f-motivo-irregular-wrap').style.display='none';
  ...
  if(document.getElementById('f-dataacordo')){document.getElementById('f-dataacordo').value='';document.getElementById('f-dataacordo').disabled=false;}
  if(document.getElementById('f-dataacordo-ni'))document.getElementById('f-dataacordo-ni').checked=false;
...
// ===== NORMALIZAÇÃO DE NOMES DE PRESIDENTE =====
// Evita duplicidade por acento, caixa, espaços extras, pontuação etc.
function normPresidenteKey(nome){
  if(!nome) return '';
  return String(nome)
    .normalize('NFD').replace(/[\u0300-\u036f]/g,'')   // remove acentos
    .toLowerCase()
    .replace(/[^a-z0-9\s]/g,' ')                       // remove pontuação
    .replace(/\b(dr|sr|sra|prof|presidente|pres)\b\.?/g,'') // títulos comuns
    .replace(/\s+/g,' ')                               // colapsa espaços
    .trim();
}
// Devolve a forma canônica do nome: se já existe um sócio com a mesma chave normalizada,
// reutiliza a grafia mais frequente. Senão devolve o próprio nome (com title case leve).
function canonPresidente(nomeBruto){
  const nome = (nomeBruto||'').trim().replace(/\s+/g,' ');
  if(!nome) return '';
  const key = normPresidenteKey(nome);
  if(!key) return nome;
  const counts = {};
  (typeof socios!=='undefined' ? socios : []).forEach(s=>{
    const p = (s.presidente||'').trim();
    if(!p) return;
    if(normPresidenteKey(p) !== key) return;
    counts[p] = (counts[p]||0) + 1;
  });
  const existentes = Object.entries(counts).sort((a,b)=>b[1]-a[1]);
  if(existentes.length) return existentes[0][0]; // reutiliza grafia já dominante
  // Title case leve (preserva preposições em minúsculo)
  const minusculas = new Set(['de','da','do','das','dos','e']);
  return nome.split(' ').map((w,i)=>{
    const lw = w.toLowerCase();
    if(i>0 && minusculas.has(lw)) return lw;
    return lw.charAt(0).toUpperCase()+lw.slice(1);
  }).join(' ');
}
// Atualiza o <datalist> de sugestões de presidente com base nos sócios já cadastrados
function atualizarListaPresidentes(){
  const dl = document.getElementById('lista-presidentes');
  if(!dl) return;
  const set = new Map(); // key -> nome canônico
  (socios||[]).forEach(s=>{
    const p = (s.presidente||'').trim();
    if(!p) return;
    const k = normPresidenteKey(p);
    if(!set.has(k)) set.set(k, p);
  });
  dl.innerHTML = Array.from(set.values()).sort().map(n=>`<option value="${sanitize(n)}"></option>`).join('');
}
// Mostra dica em tempo real avisando se o nome digitado bate com um presidente já existente
function sugerirPresidente(input, hintId){
  const hint = document.getElementById(hintId);
  if(!hint) return;
  const val = (input.value||'').trim();
  if(!val){ hint.textContent='Presidente em exercício na data de ingresso. Nomes parecidos são unificados automaticamente.'; hint.style.color='#7c3aed'; return; }
  const key = normPresidenteKey(val);
  const existentes = new Set();
  (socios||[]).forEach(s=>{
    const p=(s.presidente||'').trim();
    if(p && normPresidenteKey(p)===key) existentes.add(p);
  });
  if(existentes.size){
    const canonico = canonPresidente(val);
    if(canonico !== val){
      hint.innerHTML = `↪ Será unificado com <strong>${sanitize(canonico)}</strong> (${existentes.size} sócio(s) já cadastrado(s)).`;
      hint.style.color = '#16a34a';
    } else {
      hint.innerHTML = `✓ Reaproveitando gestão já cadastrada (${existentes.size} sócio(s)).`;
      hint.style.color = '#16a34a';
    }
  } else {
    hint.innerHTML = '⚑ Novo presidente — será criada uma nova gestão.';
...
function origemLabel(o){return {transferencia:'Transferência',heranca:'Herança / Causa Mortis',adquirido_clube:'Adquirido diretamente do Clube',compra:'Compra/Venda',familiar:'Familiar (isento)',outro:'Outro'}[o]||o||'—';}
function sitLabel(s){return{ativo:'Ativo',suspenso:'Suspenso',inadimplente:'Inadimplente',irregular:'Irregular',desligado:'Desligado'}[s]||s;}
function sitTag(s){const c={ativo:'tag-ativo',suspenso:'tag-suspenso',inadimplente:'tag-inadimplente',irregular:'tag-irregular',desligado:'tag-desligado'};return '<span class="tag '+(c[s]||'tag-ativo')+'">'+sitLabel(s)+'</span>';}

// ===== STATUS IRREGULAR (v5) =====
function isIrregular(s){ return getSitNorm(s) === 'irregular'; }
function onSituacaoChange(){
  const v = document.getElementById('f-situacao')?.value;
  const w = document.getElementById('f-motivo-irregular-wrap');
  if (w) w.style.display = (v === 'irregular') ? '' : 'none';
}
function onSituacaoEditChange(){
  const v = document.getElementById('e-situacao')?.value;
  const w = document.getElementById('e-motivo-irregular-wrap');
  if (w) w.style.display = (v === 'irregular') ? '' : 'none';
}
window.onSituacaoChange = onSituacaoChange;
window.onSituacaoEditChange = onSituacaoEditChange;
...
function renderSocios(resetPag) {
  ...
  // Drill especial: apenas sócios com pelo menos 1 dependente irregular (Art. 22)
  if(window._drillDepIrregular){
    filtered = filtered.filter(s => Array.isArray(s.dependentes) && s.dependentes.some(d => {
      try { return avaliarDependente(d, s).status === 'irregular'; }
      catch(e){ return String(d?.statusArt22||d?.status||'').toLowerCase()==='irregular'; }
    }));
  }
  filtered.sort((a,b)=>Number(a.matricula)-Number(b.matricula));
  ...
  const drillBanner = window._drillDepIrregular ? `<div style="display:flex;align-items:center;justify-content:space-between;gap:8px;background:#fee2e2;color:#991b1b;border:1px solid #fca5a5;border-radius:8px;padding:8px 12px;margin:6px 0 10px;font-size:12px;font-weight:600">⚠ Mostrando apenas sócios com dependentes irregulares (Art. 22) <button class="btn-outline" style="padding:3px 10px;font-size:11px" onclick="window._drillDepIrregular=false;renderSocios();">Limpar filtro</button></div>` : '';
  const paginacaoHtml=totalPags>1?`<div style="display:flex;align-items:center;gap:8px;margin:8px 0 4px;flex-wrap:wrap">
    ${_sociosPag>1?'<button type="button" class="btn-outline" style="padding:4px 10px;font-size:12px" onclick="window.paginarSocios(-1)">← Anterior</button>':''}
    <span style="font-size:12px;color:#6b7280;padding:0 8px">P\u00e1g ${_sociosPag} de ${totalPags} &middot; ${total} s\u00f3cios</span>
    ${_sociosPag<totalPags?'<button type="button" class="btn-outline" style="padding:4px 10px;font-size:12px" onclick="window.paginarSocios(1)">Pr\u00f3xima \u2192</button>':''}
  </div>`:'';
  list.innerHTML=drillBanner+paginacaoHtml+pagina.map(s=>{
...
function paginarSocios(dir){
  _sociosPag = Math.max(1, _sociosPag + Number(dir||0));
  window.renderSocios(false);
  const anchor = document.getElementById('tab-socios');
  if(anchor && typeof anchor.offsetTop==='number') window.scrollTo({top:anchor.offsetTop, behavior:'smooth'});
}
renderSocios._pag=paginarSocios;
window.renderSocios = renderSocios;
window.paginarSocios = paginarSocios;
...
function btcCardHTML(pessoa, matricula, isDependent, tipoSocio) {
  ...
  return `<div class="btc-card">
    ...
    <div class="btc-footer">VÁLIDA ATÉ ${anoAtual} · PRESIDENTE ADMINISTRATIVO</div>
  </div>`;
}
...
function printCards(){
  ...
  printCards.innerHTML=d.innerHTML;
  printArea.style.display='block';
  // Delay para o browser renderizar imagens e estilos antes de imprimir
...
function renderTitulos(){
  ...
  document.getElementById('titulos-resumo').innerHTML=[
    ...
    {icon:'',num:props.filter(s=>s.origemTitulo==='adquirido_clube').length,label:'Adquiridos do Clube'},
  ...
  list.innerHTML=filtered.map(s=>`
    <div class="titulo-row">
      ...
      <div style="flex:1">
        ...
        <div style="font-size:11px;color:#9ca3af;margin-top:3px">Origem: ${sanitize(origemLabel(s.origemTitulo))} · Desde: ${sanitize(s.dataTitulo||s.dataCadastro||'')}</div>
...
// Estado dos filtros globais (persistente durante a sessão)
let dashFilters = { periodo:'all', tipo:'', sit:'', bairro:'' };
let dashActivePane = 'visao';
let presTableSort = { col:'total', dir:'desc' };

// ---------- HELPERS DE DATA ----------
function _parseDataCad(d){
  if(!d) return null;
  // aceita yyyy-mm-dd, dd/mm/yyyy, etc.
  let m = String(d).match(/(\d{4})-(\d{1,2})-(\d{1,2})/);
  if(m) return new Date(+m[1], +m[2]-1, +m[3]);
  m = String(d).match(/(\d{1,2})\/(\d{1,2})\/(\d{4})/);
  if(m) return new Date(+m[3], +m[2]-1, +m[1]);
  m = String(d).match(/(\d{4})/);
  if(m) return new Date(+m[1], 0, 1);
  return null;
}
// Data de referência para o dashboard analítico:
// usa "Data do Acordo / Ingresso" (entrada real no clube) quando disponível;
// quando ausente, recai para a data de cadastro no sistema.
function _dataRef(s){
  if(s && s.dataAcordo){
    const d = _parseDataCad(s.dataAcordo);
    if(d) return d;
  }
  return _parseDataCad(s && s.dataCadastro);
}
function _idadeDe(ds){
  if(!ds) return null;
  const d = _parseDataCad(ds); if(!d) return null;
  const hj = new Date();
  let i = hj.getFullYear() - d.getFullYear();
  if(hj.getMonth() < d.getMonth() || (hj.getMonth()===d.getMonth() && hj.getDate()<d.getDate())) i--;
  return (i>=0 && i<=120) ? i : null;
}
function _faixaIdade(i){
  if(i==null) return null;
  if(i<=17) return '0-17';
  if(i<=30) return '18-30';
  if(i<=50) return '31-50';
  if(i<=70) return '51-70';
  return '71+';
}

// ---------- APLICAÇÃO DOS FILTROS ----------
function _periodoRange(key){
  const hj = new Date();
  if(key==='12m') return { ini: new Date(hj.getFullYear(), hj.getMonth()-12, hj.getDate()), fim: hj, label:'Últimos 12 meses' };
  if(key==='ytd') return { ini: new Date(hj.getFullYear(),0,1), fim: hj, label:'Ano atual ('+hj.getFullYear()+')' };
  if(key==='prev'){
    const y = hj.getFullYear()-1;
    return { ini: new Date(y,0,1), fim: new Date(y,11,31), label:'Ano '+y };
  }
  return null;
}
function _periodoAnterior(key){
  const r = _periodoRange(key); if(!r) return null;
  const dur = r.fim - r.ini;
  return { ini: new Date(r.ini.getTime() - dur), fim: new Date(r.ini.getTime() - 1), label:'Período anterior' };
}
function _filtrar(base){
  let arr = base.slice();
  const f = dashFilters;
  if(f.tipo)   arr = arr.filter(s => getTipoNorm(s) === f.tipo);
  if(f.sit)    arr = arr.filter(s => getSitNorm(s)  === f.sit);
  if(f.bairro) arr = arr.filter(s => (s.bairro||'').trim() === f.bairro);
  if(f.periodo && f.periodo!=='all'){
    const r = _periodoRange(f.periodo);
    if(r) arr = arr.filter(s => { const d=_dataRef(s); return d && d>=r.ini && d<=r.fim; });
  }
  return arr;
}

function aplicarFiltrosDash(){
  dashFilters.periodo = document.getElementById('filt-periodo')?.value || 'all';
  dashFilters.tipo    = document.getElementById('filt-tipo')?.value    || '';
  dashFilters.sit     = document.getElementById('filt-sit')?.value     || '';
  dashFilters.bairro  = document.getElementById('filt-bairro')?.value  || '';
  renderDashboard();
}
function limparFiltrosDash(){
  dashFilters = { periodo:'all', tipo:'', sit:'', bairro:'' };
  ['filt-periodo','filt-tipo','filt-sit','filt-bairro'].forEach(id=>{const el=document.getElementById(id);if(el)el.value=el.querySelector('option')?.value||'';});
  renderDashboard();
}

// ---------- SUB-ABAS ----------
function showDashPane(name){
  dashActivePane = name;
  document.querySelectorAll('.dash-pane').forEach(p => p.style.display = (p.id==='pane-'+name)?'block':'none');
  document.querySelectorAll('.dash-subtab').forEach(b => b.classList.toggle('active', b.dataset.pane===name));
}

// ---------- RENDERIZAÇÕES SVG ----------
const PALETTE = ['#1B3A70','#3b82f6','#0891b2','#10b981','#f59e0b','#ef4444','#7c3aed','#ec4899','#64748b'];

function renderDonut(elId, data){
  const el = document.getElementById(elId); if(!el) return;
  const total = data.reduce((a,d)=>a+d.v,0);
  if(!total){ el.innerHTML = '<p style="color:#9ca3af;font-size:13px;padding:20px">Sem dados.</p>'; return; }
  const cx=110, cy=110, r=85, rIn=55;
  let acc = 0;
  const arcs = data.filter(d=>d.v>0).map((d,i) => {
    ...
    return `<path d="M ${x1} ${y1} A ${r} ${r} 0 ${large} 1 ${x2} ${y2} L ${xi1} ${yi1} A ${rIn} ${rIn} 0 ${large} 0 ${xi2} ${yi2} Z" fill="${PALETTE[i%PALETTE.length]}"><title>${sanitize(d.l)}: ${d.v} (${Math.round(d.v/total*100)}%)</title></path>`;
  }).join('');
  const legend = data.filter(d=>d.v>0).map((d,i) =>
    `<div style="display:flex;align-items:center;gap:7px;font-size:12px;color:#374151;margin:3px 0"><span style="width:12px;height:12px;background:${PALETTE[i%PALETTE.length]};border-radius:2px;flex-shrink:0"></span><span style="flex:1">${sanitize(d.l)}</span><strong>${d.v}</strong> <span style="color:#9ca3af">(${Math.round(d.v/total*100)}%)</span></div>`
  ).join('');
  el.innerHTML = `<div style="display:flex;gap:20px;align-items:center;flex-wrap:wrap;justify-content:center">
    <svg width="220" height="220" viewBox="0 0 220 220">${arcs}<text x="110" y="105" text-anchor="middle" font-size="13" fill="#6b7280" font-weight="600">Total</text><text x="110" y="128" text-anchor="middle" font-size="22" fill="#1B3A70" font-weight="800">${total}</text></svg>
    <div style="min-width:180px;flex:1">${legend}</div>
  </div>`;
}

function renderSparkline(elId, points, labels){
  const el = document.getElementById(elId); if(!el) return;
  if(!points.length){ el.innerHTML = '<p style="color:#9ca3af;font-size:13px;padding:20px">Sem dados.</p>'; return; }
  const W=460, H=160, pad=24;
  const max = Math.max(...points, 1);
  const dx = (W-pad*2) / Math.max(points.length-1,1);
  const pts = points.map((v,i) => [pad + i*dx, H-pad - (v/max)*(H-pad*2)]);
  const path = pts.map((p,i)=> (i?'L':'M') + p[0].toFixed(1) + ',' + p[1].toFixed(1)).join(' ');
  const area = path + ` L ${pts[pts.length-1][0].toFixed(1)},${H-pad} L ${pts[0][0].toFixed(1)},${H-pad} Z`;
  const dots = pts.map((p,i)=>`<circle cx="${p[0].toFixed(1)}" cy="${p[1].toFixed(1)}" r="3" fill="#1B3A70"><title>${labels[i]}: ${points[i]}</title></circle>`).join('');
  const labs = labels.map((l,i)=> i%Math.ceil(labels.length/6)===0 ? `<text x="${pts[i][0].toFixed(1)}" y="${H-6}" text-anchor="middle" font-size="10" fill="#6b7280">${l}</text>` : '').join('');
  el.innerHTML = `<svg width="100%" viewBox="0 0 ${W} ${H}" preserveAspectRatio="xMidYMid meet">
    <path d="${area}" fill="rgba(27,58,112,.12)"/>
    <path d="${path}" fill="none" stroke="#1B3A70" stroke-width="2.5" stroke-linejoin="round"/>
    ${dots}${labs}
  </svg>`;
}

function renderPiramide(elId, dataM, dataF, labels){
  const el = document.getElementById(elId); if(!el) return;
  const max = Math.max(...dataM, ...dataF, 1);
  // Layout em colunas reservadas: [numM][barraM][faixa][barraF][numF]
  const numW   = 36;   // coluna do número (sempre fora da barra)
  const labW   = 64;   // coluna central com a faixa etária
  const barMax = 200;  // largura máxima de cada barra
  const rowH   = 32;
  const gap    = 8;
  const padTop = 28;   // espaço para títulos MASCULINO / FEMININO
  const W = numW*2 + barMax*2 + labW;
  const H = labels.length * (rowH+gap) + padTop + 8;
  // Centros das colunas
  const xNumM = numW - 6;                    // text-anchor end → número masc à esquerda
  const xBarMend = numW + barMax;            // ponta direita da área masc (barra cresce p/ a esquerda)
  const xLabCenter = numW + barMax + labW/2; // centro da faixa
  const xBarFstart = numW + barMax + labW;   // início da área fem
  const xNumF = xBarFstart + barMax + 6;     // text-anchor start → número fem à direita

  const rows = labels.map((l,i)=>{
    const y = padTop + i*(rowH+gap);
    const wM = (dataM[i]/max) * barMax;
    const wF = (dataF[i]/max) * barMax;
    const xM = xBarMend - wM; // barra masc cresce do meio para a esquerda
    const yMid = y + rowH/2 + 4;
    return `
      <text x="${xNumM}" y="${yMid}" text-anchor="end" font-size="12" fill="#1f2937" font-weight="700">${dataM[i]||''}</text>
      <rect x="${xM}" y="${y}" width="${wM}" height="${rowH}" fill="#1B3A70" rx="4"><title>Masculino ${l}: ${dataM[i]}</title></rect>
      <text x="${xLabCenter}" y="${yMid}" text-anchor="middle" font-size="12" fill="#374151" font-weight="700">${l}</text>
      <rect x="${xBarFstart}" y="${y}" width="${wF}" height="${rowH}" fill="#ec4899" rx="4"><title>Feminino ${l}: ${dataF[i]}</title></rect>
      <text x="${xNumF}" y="${yMid}" text-anchor="start" font-size="12" fill="#1f2937" font-weight="700">${dataF[i]||''}</text>
    `;
  }).join('');
  el.innerHTML = `<svg width="100%" viewBox="0 0 ${W} ${H}" preserveAspectRatio="xMidYMid meet">
    <text x="${numW + barMax/2}" y="16" text-anchor="middle" font-size="12" fill="#1B3A70" font-weight="700" letter-spacing="1">MASCULINO</text>
    <text x="${xBarFstart + barMax/2}" y="16" text-anchor="middle" font-size="12" fill="#ec4899" font-weight="700" letter-spacing="1">FEMININO</text>
    <line x1="${xLabCenter}" y1="${padTop-4}" x2="${xLabCenter}" y2="${H-2}" stroke="#e5e7eb" stroke-width="1"/>
    ${rows}
  </svg>`;
}

function renderArea(elId, labels, values){
  const el = document.getElementById(elId); if(!el) return;
  if(!values.length){ el.innerHTML = '<p style="color:#9ca3af;font-size:13px;padding:20px">Sem dados.</p>'; return; }
  ...
  const grid = [0,.25,.5,.75,1].map(t=>{
    ...
    return `<line x1="${pad}" y1="${y}" x2="${W-pad}" y2="${y}" stroke="#e5e7eb" stroke-dasharray="2 3"/><text x="${pad-6}" y="${y+3}" text-anchor="end" font-size="10" fill="#9ca3af">${Math.round(max*t)}</text>`;
  }).join('');
  const dots = pts.map((p,i)=>`<circle cx="${p[0].toFixed(1)}" cy="${p[1].toFixed(1)}" r="4" fill="#1B3A70"><title>${labels[i]}: ${values[i]}</title></circle>`).join('');
  const labs = labels.map((l,i)=>`<text x="${pts[i][0].toFixed(1)}" y="${H-12}" text-anchor="middle" font-size="10" fill="#6b7280">${l}</text>`).join('');
  el.innerHTML = `<svg width="100%" viewBox="0 0 ${W} ${H}" preserveAspectRatio="xMidYMid meet">
    ${grid}
    <path d="${area}" fill="rgba(27,58,112,.15)"/>
    <path d="${path}" fill="none" stroke="#1B3A70" stroke-width="2.5"/>
    ${dots}${labs}
  </svg>`;
}

function renderHeatmap(elId, entries){
  const el = document.getElementById(elId); if(!el) return;
  if(!entries.length){ el.innerHTML = '<p style="color:#9ca3af;font-size:13px">Sem dados.</p>'; return; }
  const max = entries[0][1];
  const total = entries.reduce((a,e)=>a+e[1],0);
  el.innerHTML = entries.map(([l,v])=>{
    const intensity = v/max;
    const bg = `rgba(27,58,112,${0.10 + intensity*0.65})`;
    const color = intensity>0.55?'#fff':'#1f2937';
    return `<div class="heat-row" style="background:${bg};color:${color}"><span class="heat-lbl">${sanitize(l)}</span><span class="heat-val">${v}</span><span class="heat-pct" style="color:${color};opacity:.85">${Math.round(v/total*100)}%</span></div>`;
  }).join('');
}

// ---------- BARRAS LEGADAS ----------
function _bar(id, data, baseTotal){
  const el = document.getElementById(id); if(!el) return;
  const base = baseTotal || data.reduce((a,d)=>a+d.v,0);
...
function renderDashboard(){
  ...
  const ts = 'Atualizado em ' + agora.toLocaleDateString('pt-BR') + ' às ' + agora.toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'});
  ...
  // Popular dropdown de bairros (uma vez)
  const fb = document.getElementById('filt-bairro');
  if(fb && fb.options.length<=1){
    const bSet = {};
    socios.forEach(s => { if(s.bairro) bSet[s.bairro.trim()] = (bSet[s.bairro.trim()]||0)+1; });
    Object.entries(bSet).sort((a,b)=>b[1]-a[1]).slice(0,30).forEach(([b])=>{
      const o=document.createElement('option'); o.value=b; o.textContent=b; fb.appendChild(o);
    });
  }

  // Aplicar filtros
  const arr = _filtrar(socios);
  const total = arr.length;

  // Status dos filtros ativos
  const fStatus = document.getElementById('filt-status');
  if(fStatus){
    const ativos = [];
    if(dashFilters.periodo!=='all') ativos.push(_periodoRange(dashFilters.periodo)?.label);
    if(dashFilters.tipo)   ativos.push('Tipo: '+dashFilters.tipo);
    if(dashFilters.sit)    ativos.push('Situação: '+dashFilters.sit);
    if(dashFilters.bairro) ativos.push('Bairro: '+dashFilters.bairro);
    fStatus.innerHTML = ativos.length
      ? `<span style="color:var(--gray);font-weight:500">Filtros ativos:</span> ${ativos.map(a=>'<span style="background:#dbeafe;padding:2px 8px;border-radius:10px;margin-right:4px">'+sanitize(String(a))+'</span>').join('')} <span style="color:var(--gray);font-weight:500;margin-left:6px">→ ${total} sócio(s) na seleção</span>`
      : '';
  }

  if(total===0){
    document.getElementById('dash-exec-text').innerHTML = '<p>Nenhum sócio corresponde aos filtros aplicados. Ajuste ou limpe os filtros para ver os dados.</p>';
    ['dash-alertas','dash-insight','kpi-row','chart-donut','chart-spark','chart-tipo','chart-sit','chart-piramide','chart-sexo','chart-estcivil','chart-bairro-heat','chart-ativ','chart-area','tabela-presidentes','comparativo-out','lista-resolver','lista-aniver','lista-recentes','chart-completude','chart-deps','chart-deps-motivos','score-num','score-detail']
      .forEach(id=>{const el=document.getElementById(id);if(el)el.innerHTML='';});
    return;
  }

  // ===== MÉTRICAS =====
  const props        = arr.filter(s=>getTipoNorm(s)==='proprietario').length;
  const remidos      = arr.filter(s=>getTipoNorm(s)==='remido').length;
  const benemeritos  = arr.filter(s=>getTipoNorm(s)==='benemerito').length;
  const inativos     = arr.filter(s=>getTipoNorm(s)==='inativo').length;
  const naoSocios    = arr.filter(s=>getTipoNorm(s)==='nao_socio').length;
  const conts        = arr.filter(s=>['contribuinte','contribuinte_policial','contribuinte_prefeitura'].includes(getTipoNorm(s))).length;
  const mulheres     = arr.filter(s=>s.sexo==='F').length;
  const homens       = arr.filter(s=>s.sexo==='M').length;
  const ativos       = arr.filter(s=>getSitNorm(s)==='ativo').length;
  const suspensos    = arr.filter(s=>getSitNorm(s)==='suspenso').length;
  const desligados   = arr.filter(s=>getSitNorm(s)==='desligado').length;
  const inadimplentes= arr.filter(s=>getSitNorm(s)==='inadimplente').length;
  const irregulares  = arr.filter(s=>getSitNorm(s)==='irregular').length;
  const totalDeps    = arr.reduce((a,s)=>a+(s.dependentes?.length||0),0);
  const comDeps      = arr.filter(s=>s.dependentes?.length>0).length;
  // Avaliação de regularidade dos dependentes (Art. 22)
  const depsFlat = [];
  arr.forEach(s => (s.dependentes||[]).forEach(d => depsFlat.push({d, s, av: avaliarDependente(d,s)})));
  const depsRegulares  = depsFlat.filter(x=>x.av.status==='regular').length;
  const depsPendentes  = depsFlat.filter(x=>x.av.status==='pendente').length;
  const depsIrregulares= depsFlat.filter(x=>x.av.status==='irregular').length;
  const motivosCount = {};
  depsFlat.filter(x=>x.av.status==='irregular').forEach(x=>{
    x.av.motivos.forEach(m=>{
      const chave = m.replace(/com \d+/g,'com X').replace(/\d+\.\d+/g,'X').slice(0,80);
      motivosCount[chave] = (motivosCount[chave]||0)+1;
    });
  });

  // Bairros
  const bairros = {};
  arr.forEach(s=>{ if(s.bairro){ const b=s.bairro.trim(); if(b) bairros[b]=(bairros[b]||0)+1; }});
  const bairrosTop = Object.entries(bairros).sort((a,b)=>b[1]-a[1]);

  // Idade
  const idades = arr.map(s=>_idadeDe(s.dataNasc)).filter(i=>i!==null);
  const faixas = {'0-17':0,'18-30':0,'31-50':0,'51-70':0,'71+':0};
  idades.forEach(i=>{ faixas[_faixaIdade(i)]++; });
  ...
  const fOrder = ['0-17','18-30','31-50','51-70','71+'];
  const piramM = fOrder.map(f => arr.filter(s=>s.sexo==='M' && _faixaIdade(_idadeDe(s.dataNasc))===f).length);
  const piramF = fOrder.map(f => arr.filter(s=>s.sexo==='F' && _faixaIdade(_idadeDe(s.dataNasc))===f).length);

  // Evolução por ano
  const porAno = {};
  arr.forEach(s=>{ const d=_dataRef(s); if(d){ porAno[d.getFullYear()]=(porAno[d.getFullYear()]||0)+1; }});
  const anosOrd = Object.keys(porAno).sort();
  const anoMaisAtivo = anosOrd.reduce((m,a)=>(porAno[a]>(porAno[m]||0)?a:m), anosOrd[0]);
  ...
  for(let i=11;i>=0;i--){
    ...
    meses12.push({y:d.getFullYear(), m:d.getMonth(), label: d.toLocaleDateString('pt-BR',{month:'short'}).replace('.','')});
  }
  const sparkVals = meses12.map(mm => arr.filter(s=>{const d=_dataRef(s);return d && d.getFullYear()===mm.y && d.getMonth()===mm.m;}).length);
  const sparkLabels = meses12.map(mm => mm.label);

  // Completude
  const semCPF=arr.filter(s=>!s.cpf).length, semTel=arr.filter(s=>!(s.tel||s.telefone)).length;
  const semEnd=arr.filter(s=>!s.endereco).length, semEmail=arr.filter(s=>!s.email).length;
  const semBairro=arr.filter(s=>!s.bairro).length;
  const pctCompleto = Math.round(((total-semCPF)/total)*100);

  // Score de saúde (média ponderada de preenchimentos críticos)
  const camposScore = [
    {nome:'CPF',         peso:25, ok: total-semCPF},
    {nome:'Telefone',    peso:20, ok: total-semTel},
    {nome:'Endereço',    peso:15, ok: total-semEnd},
    {nome:'Bairro',      peso:10, ok: total-semBairro},
    {nome:'Data nasc.',  peso:15, ok: total-semDataNasc},
    {nome:'E-mail',      peso:5,  ok: total-semEmail},
    {nome:'Sem irregularidade', peso:10, ok: total-irregulares},
  ];
  const score = Math.round(camposScore.reduce((a,c)=>a + (c.ok/total)*c.peso, 0));

  // Tendência (compara período atual vs anterior usando dataCadastro) — só quando há filtro de período
  function _trend(metricFn){
    if(dashFilters.periodo==='all') return null;
    const cur = _periodoRange(dashFilters.periodo);
    const prev = _periodoAnterior(dashFilters.periodo);
    if(!cur||!prev) return null;
    const inRange = (s,r) => { const d=_dataRef(s); return d && d>=r.ini && d<=r.fim; };
    const baseCur  = socios.filter(s=>inRange(s,cur));
    const basePrev = socios.filter(s=>inRange(s,prev));
    const a = metricFn(baseCur), b = metricFn(basePrev);
    if(b===0 && a===0) return {dir:'flat', pct:0};
    if(b===0) return {dir:'up', pct:100};
    const pct = Math.round(((a-b)/b)*100);
    return { dir: pct>2?'up':pct<-2?'down':'flat', pct };
  }
  const trendNovos = _trend(b=>b.length);
  const trendProps = _trend(b=>b.filter(s=>getTipoNorm(s)==='proprietario').length);
  const trendAtivos= _trend(b=>b.filter(s=>getSitNorm(s)==='ativo').length);
  ...
  if(execEl){
    ...
    const topBairro = bairrosTop[0];
    execEl.innerHTML = `
      <p>O recorte atual contempla <strong>${total} sócio(s)</strong>, dos quais <strong>${ativos} ativo(s) (${pctAtivos}%)</strong>${suspensos?`, ${suspensos} suspenso(s)`:''}${desligados?`, ${desligados} desligado(s)`:''}${inadimplentes?`, ${inadimplentes} inadimplente(s) (${pctInad}%)`:''}${irregulares?` e ${irregulares} irregular(es)`:''}.</p>
      <p>Composição: <strong>${props} proprietário(s) (${pctProps}%)</strong>, ${remidos} remido(s), ${benemeritos} benemérito(s), ${conts} contribuinte(s), ${naoSocios} não-sócio(s) e ${inativos} inativo(s). Há <strong>${totalDeps} dependente(s)</strong> vinculados a ${comDeps} titular(es).</p>
      ${idadeMedia?`<p>Idade média: <strong>${idadeMedia} anos</strong>. Faixa dominante: ${Object.entries(faixas).sort((a,b)=>b[1]-a[1])[0][0]} anos.${semDataNasc?` <em>${semDataNasc} sem data de nascimento.</em>`:''}</p>`:''}
      ${topBairro?`<p>Bairro líder: <strong>${sanitize(topBairro[0])}</strong> com ${topBairro[1]} sócio(s) (${Math.round((topBairro[1]/total)*100)}%).</p>`:''}
      <p style="font-size:12px;color:var(--gray);margin-top:8px"><em>Integridade cadastral: ${pctCompleto}% com CPF · Score de saúde: ${score}/100.</em></p>
    `;
  }
  ...
  if(alertasEl){
    const alertas = [];
    if(inadimplentes>0) alertas.push({tipo:'critico',icon:'⚠️',titulo:'Inadimplência',texto:`${inadimplentes} sócio(s) inadimplente(s) (${Math.round((inadimplentes/total)*100)}%). Veja em "Qualidade & Ações".`});
    if(irregulares>0) alertas.push({tipo:'critico',icon:'📋',titulo:'Documentação Irregular',texto:`${irregulares} sócio(s) sem documentação completa.`});
    if(semCPF>total*0.1) alertas.push({tipo:'atencao',icon:'🪪',titulo:'CPF faltante',texto:`${semCPF} sócio(s) (${Math.round((semCPF/total)*100)}%) sem CPF.`});
    if(semTel>total*0.2) alertas.push({tipo:'atencao',icon:'📞',titulo:'Contato faltante',texto:`${semTel} sócio(s) sem telefone — comunicação prejudicada.`});
    if(idadeMedia && idadeMedia>=60) alertas.push({tipo:'info',icon:'👥',titulo:'Perfil etário',texto:`Idade média de ${idadeMedia} anos sugere foco em renovação.`});
    if(!alertas.length) alertas.push({tipo:'info',icon:'✅',titulo:'Base saudável',texto:'Nenhum alerta crítico identificado.'});
  ...
  // ===== INSIGHT DO DIA =====
  const insEl = document.getElementById('dash-insight');
  if(insEl){
    const insights = [];
    // Tendência
    if(trendNovos && trendNovos.dir!=='flat'){
      insights.push(`Os novos cadastros ${trendNovos.dir==='up'?'<strong>cresceram</strong>':'<strong>caíram</strong>'} ${Math.abs(trendNovos.pct)}% comparado ao período anterior — ${trendNovos.dir==='up'?'tendência positiva de captação.':'aten­ção: ritmo de entrada em queda.'}`);
    }
    // Renovação geracional
    const novos1830 = arr.filter(s=>{const i=_idadeDe(s.dataNasc);return i!==null && i<=30;}).length;
    if(idades.length && novos1830/idades.length > 0.25){
      insights.push(`<strong>${Math.round(novos1830/idades.length*100)}%</strong> dos sócios com idade conhecida têm <strong>até 30 anos</strong> — sinal de renovação do quadro.`);
    } else if(idades.length && novos1830/idades.length < 0.10){
      insights.push(`Apenas <strong>${Math.round(novos1830/idades.length*100)}%</strong> dos sócios têm até 30 anos — quadro com perfil etário envelhecido.`);
    }
    // Concentração geográfica
    if(bairrosTop[0] && bairrosTop[0][1]/total > 0.30){
      insights.push(`<strong>${Math.round(bairrosTop[0][1]/total*100)}%</strong> dos sócios estão em <strong>${sanitize(bairrosTop[0][0])}</strong> — alta concentração geográfica.`);
    }
    // Razão M/F
    if(homens && mulheres){
      const r = homens/mulheres;
      if(r>2.5) insights.push(`Quadro fortemente masculino (<strong>${r.toFixed(1)} homens para cada mulher</strong>) — espaço para campanhas de associação feminina.`);
      else if(r<0.5) insights.push(`Quadro majoritariamente feminino (<strong>${(1/r).toFixed(1)} mulheres para cada homem</strong>).`);
    }
    // Dependentes
    if(comDeps && totalDeps/comDeps > 2.5){
      insights.push(`Cada titular com dependentes registra em média <strong>${(totalDeps/comDeps).toFixed(1)} dependentes</strong> — perfil familiar marcante.`);
    }
    // Irregularidades em dependentes (insight central de auditoria)
    if (depsIrregulares > 0) {
      const pct = totalDeps ? Math.round(depsIrregulares/totalDeps*100) : 0;
      // Ranking por presidente
      const porPres = {};
      depsFlat.filter(x=>x.av.status==='irregular').forEach(x=>{
        const p = x.s.presidente || 'Sem presidente registrado';
        const k = normPresidenteKey(p) || p;
        if(!porPres[k]) porPres[k] = {nome:p, count:0};
        porPres[k].count++;
      });
      const topPres = Object.values(porPres).sort((a,b)=>b.count-a.count)[0];
      insights.unshift(`<strong>${depsIrregulares} dependente(s) irregular(es)</strong> (${pct}% do total) violam o Art. 22 do Estatuto.${topPres?` Gestão com mais ocorrências: <strong>${sanitize(topPres.nome)}</strong> com ${topPres.count} caso(s).`:''}`);
    }
    insEl.innerHTML = insights.length ? insights[0] : 'Base equilibrada nos principais indicadores. Continue acompanhando a evolução mensal.';
  }
  ...
  function trendBadge(t){
    ...
    return `<span class="kpi-trend ${t.dir}" title="vs período anterior">${arrow} ${t.pct>0?'+':''}${t.pct}%</span>`;
  }
  const kpis = [
    {num:total,         lbl:'Total na seleção',  filtro:'',             tone:'neutral', t:trendNovos},
    {num:ativos,        lbl:'Ativos',            filtro:'ativo',        tone:'good',    t:trendAtivos},
    {num:props,         lbl:'Proprietários',     filtro:'proprietario', tone:'neutral', t:trendProps},
    {num:inadimplentes, lbl:'Inadimplentes',     filtro:'inadimplente', tone:inadimplentes?'bad':'good'},
    {num:irregulares,   lbl:'Irregulares',       filtro:'irregular',    tone:irregulares?'warn':'good'},
    {num:totalDeps,     lbl:'Dependentes',       filtro:'',             tone:'neutral'},
    {num:depsIrregulares, lbl:'Dep. Irregulares (Art. 22)', filtro:'__dep_irregular__', tone:depsIrregulares?'bad':'good'},
  ];
  document.getElementById('kpi-row').innerHTML = kpis.map(k=>`
    <div class="kpi-card" data-tone="${k.tone}" onclick="drillKpi('${escAttr(k.filtro)}')" title="${k.filtro?'Clique para filtrar sócios':''}">
  ...
  // ===== VISÃO GERAL: Donut + Sparkline =====
  renderDonut('chart-donut', [
    {l:'Proprietários', v:props},
    {l:'Remidos',       v:remidos},
    {l:'Beneméritos',   v:benemeritos},
    {l:'Contribuintes', v:conts},
    {l:'Não-sócios',    v:naoSocios},
    {l:'Inativos',      v:inativos},
  ]);
  document.getElementById('donut-leitura').textContent = props
    ? `${Math.round(props/total*100)}% do quadro são proprietários — base patrimonial do clube.`
    : 'Sem proprietários na seleção atual.';

  renderSparkline('chart-spark', sparkVals, sparkLabels);
  const totalSpark = sparkVals.reduce((a,b)=>a+b,0);
  const mediaSpark = (totalSpark/12).toFixed(1);
  document.getElementById('spark-leitura').textContent = `${totalSpark} cadastro(s) nos últimos 12 meses · média de ${mediaSpark}/mês.`;

  // Barras tipo / situação
  _bar('chart-tipo',[
    {l:'Proprietário',v:props},
    {l:'Remido',v:remidos},
    {l:'Benemérito',v:benemeritos},
    {l:'Contribuinte',v:arr.filter(s=>getTipoNorm(s)==='contribuinte').length},
    {l:'Contrib. Policial',v:arr.filter(s=>getTipoNorm(s)==='contribuinte_policial').length},
    {l:'Contrib. Prefeitura',v:arr.filter(s=>getTipoNorm(s)==='contribuinte_prefeitura').length},
    {l:'Não-sócio',v:naoSocios},
    {l:'Inativo',v:inativos},
  ].filter(d=>d.v>0));
  const sitMap = {ativo:'Ativo',suspenso:'Suspenso',inadimplente:'Inadimplente',irregular:'Irregular',desligado:'Desligado'};
  _bar('chart-sit', Object.entries(sitMap).map(([k,l])=>({l,v:arr.filter(s=>getSitNorm(s)===k).length})).filter(d=>d.v>0));

  // ===== DEMOGRAFIA =====
  const faixaDom = Object.entries(faixas).sort((a,b)=>b[1]-a[1])[0];
  const ms = (id, n, sub) => { const e=document.getElementById(id); if(e) e.textContent=n; const e2=document.getElementById(id+'-sub'); if(e2) e2.textContent=sub||''; };
  if(faixaDom && faixaDom[1]){ ms('ms-faixa', faixaDom[0]+' anos', `${faixaDom[1]} sócio(s) · ${Math.round(faixaDom[1]/idades.length*100)}% dos com idade conhecida`); }
  else ms('ms-faixa','—','Sem datas de nascimento');
  if(bairrosTop[0]) ms('ms-bairro', bairrosTop[0][0], `${bairrosTop[0][1]} sócio(s) · ${Math.round(bairrosTop[0][1]/total*100)}% do total`);
  else ms('ms-bairro','—','Sem bairros informados');
  if(homens||mulheres){
    const r = mulheres ? (homens/mulheres) : Infinity;
    ms('ms-razao', isFinite(r)?r.toFixed(2)+':1':'—', `${homens} M · ${mulheres} F`);
  } else ms('ms-razao','—','Sem informação de sexo');

  renderPiramide('chart-piramide', piramM, piramF, fOrder);
  const obsIdade = document.getElementById('idade-obs');
  if(obsIdade){
    obsIdade.textContent = idades.length
      ? `Idade média: ${idadeMedia} anos · Base: ${idades.length} sócio(s) com data de nascimento${semDataNasc?` (${semDataNasc} sem data foram excluídos)`:''}.`
      : 'Nenhum sócio tem data de nascimento cadastrada.';
  }

  _bar('chart-sexo',[{l:'Masculino',v:homens},{l:'Feminino',v:mulheres}].filter(d=>d.v>0));
  const ec={};
  arr.forEach(s=>{ const k={solteiro:'Solteiro(a)',casado:'Casado(a)',uniaoEstavel:'União Estável',divorciado:'Divorciado(a)',viuvo:'Viúvo(a)'}[s.estadoCivil]||s.estadoCivil||'Não informado'; ec[k]=(ec[k]||0)+1; });
  _bar('chart-estcivil', Object.entries(ec).sort((a,b)=>b[1]-a[1]).map(([l,v])=>({l,v})));

  renderHeatmap('chart-bairro-heat', bairrosTop.slice(0,15));

  const ac={};
  arr.forEach(s=>{ const a=(s.atividade||'Não informado').split(/[,;\/]/); a.forEach(x=>{ const k=x.trim()||'Não informado'; ac[k]=(ac[k]||0)+1; }); });
  _bar('chart-ativ', Object.entries(ac).sort((a,b)=>b[1]-a[1]).slice(0,10).map(([l,v])=>({l,v})));

  // ===== GESTÃO & HISTÓRICO =====
  renderArea('chart-area', anosOrd, anosOrd.map(a=>porAno[a]));
  const obsEvo = document.getElementById('evolucao-obs');
  if(obsEvo){
    obsEvo.textContent = anosOrd.length
      ? `Período: ${anosOrd[0]} a ${anosOrd[anosOrd.length-1]} · Pico: ${anoMaisAtivo} (${porAno[anoMaisAtivo]} cadastros).`
      : 'Sem datas de cadastro disponíveis.';
  }

  // Tabela presidentes — agrupa por chave normalizada (evita duplicidade por acento/caixa)
  const presStats = {};
  arr.forEach(s=>{
    const p = s.presidente; if(!p) return;
    const key = normPresidenteKey(p) || p;
    if(!presStats[key]) presStats[key] = {nome:p, _key:key, _variantes:new Set([p]), total:0, props:0, remidos:0, deps:0, depsIrr:0};
    else presStats[key]._variantes.add(p);
    presStats[key].total++;
    if(getTipoNorm(s)==='proprietario') presStats[key].props++;
    if(getTipoNorm(s)==='remido')       presStats[key].remidos++;
    (s.dependentes||[]).forEach(d=>{
      presStats[key].deps++;
      if (avaliarDependente(d,s).status==='irregular') presStats[key].depsIrr++;
    });
  });
  // Para cada chave, escolhe a grafia mais frequente como nome canônico
  Object.values(presStats).forEach(p=>{
    if(p._variantes.size > 1){
      const counts = {};
      arr.forEach(s=>{ if(s.presidente && normPresidenteKey(s.presidente)===p._key) counts[s.presidente]=(counts[s.presidente]||0)+1; });
      p.nome = Object.entries(counts).sort((a,b)=>b[1]-a[1])[0][0];
      p._unificou = Array.from(p._variantes).filter(v=>v!==p.nome);
    }
  });
  renderTabelaPresidentes(Object.values(presStats));

  // Popular dropdowns de comparativo
  const presList = Object.keys(presStats).sort();
  ['cmp-pres-a','cmp-pres-b'].forEach((id,idx)=>{
    const sel = document.getElementById(id);
    if(sel && sel.options.length<=1){
      sel.innerHTML = '<option value="">— escolha —</option>' + presList.map(p=>`<option value="${sanitize(p)}">${sanitize(p)}</option>`).join('');
      if(presList[idx]) sel.value = presList[idx];
    }
  });
  renderComparativo();
  ...
  if(scoreCircle){
    scoreCircle.classList.remove('score-bom','score-medio','score-ruim');
    scoreCircle.classList.add(score>=80?'score-bom':score>=60?'score-medio':'score-ruim');
  }
  if(scoreDetail){
    const status = score>=80?'<strong>Saudável</strong> — base bem mantida.':score>=60?'<strong>Atenção</strong> — há campos críticos a completar.':'<strong>Ruim</strong> — base com lacunas significativas que comprometem operação e comunicação.';
    scoreDetail.innerHTML = `<p>${status}</p><p style="margin-top:6px;color:var(--gray)">Composição do score:</p>
      <ul>${camposScore.map(c=>`<li><strong>${c.nome}</strong> (peso ${c.peso}): ${c.ok}/${total} preenchido(s) — ${Math.round(c.ok/total*100)}%</li>`).join('')}</ul>`;
  }

  // Resolver agora — sócios ATIVOS com problemas críticos
  const problemas = arr.filter(s => getSitNorm(s)==='ativo' || getSitNorm(s)==='inadimplente').map(s => {
    const tags=[];
    if(getSitNorm(s)==='inadimplente') tags.push({c:'',t:'Inadimplente'});
    if(!s.cpf) tags.push({c:'',t:'Sem CPF'});
    if(!(s.tel||s.telefone)) tags.push({c:'warn',t:'Sem telefone'});
    if(!s.endereco) tags.push({c:'info',t:'Sem endereço'});
    if(!s.dataNasc) tags.push({c:'info',t:'Sem data nasc.'});
    return {s, tags};
  }).filter(p => p.tags.length>0).sort((a,b)=>b.tags.length-a.tags.length).slice(0,30);
  const resolverEl = document.getElementById('lista-resolver');
  if(resolverEl){
    resolverEl.innerHTML = problemas.length
      ? problemas.map(p=>`<div class="acao-row" onclick="abrirFicha('${escAttr(String(p.s.id))}')">
          <span class="acao-mat">#${sanitize(p.s.matricula||'-')}</span>
          <span class="acao-nome">${sanitize(p.s.nome)}</span>
          <span class="acao-tags">${p.tags.map(t=>`<span class="acao-tag ${t.c}">${t.t}</span>`).join('')}</span>
        </div>`).join('')
      : '<p style="color:#10b981;font-size:13px;padding:8px">✅ Nenhum sócio crítico na seleção atual.</p>';
  }

  // Aniversariantes
  const mesHj = new Date().getMonth();
  const aniver = arr.filter(s=>{ const d=_parseDataCad(s.dataNasc); return d && d.getMonth()===mesHj; })
    .sort((a,b)=>{ const da=_parseDataCad(a.dataNasc),db=_parseDataCad(b.dataNasc); return da.getDate()-db.getDate(); });
  const aniverEl = document.getElementById('lista-aniver');
  if(aniverEl){
    aniverEl.innerHTML = aniver.length
      ? aniver.map(s=>{const d=_parseDataCad(s.dataNasc);return `<div class="acao-row" onclick="abrirFicha('${escAttr(String(s.id))}')">
          <span class="acao-mat">dia ${String(d.getDate()).padStart(2,'0')}</span>
          <span class="acao-nome">${sanitize(s.nome)}</span>
          <span class="acao-tags"><span class="acao-tag info">${_idadeDe(s.dataNasc)} anos</span></span>
        </div>`;}).join('')
      : '<p style="color:#9ca3af;font-size:13px;padding:8px">Nenhum aniversariante neste mês.</p>';
  }

  // Cadastros recentes (30 dias)
  const limite = new Date(); limite.setDate(limite.getDate()-30);
  const recentes = arr.filter(s=>{const d=_parseDataCad(s.dataCadastro);return d && d>=limite;})
    .sort((a,b)=>{const da=_parseDataCad(a.dataCadastro),db=_parseDataCad(b.dataCadastro);return db-da;});
  const recEl = document.getElementById('lista-recentes');
  if(recEl){
    recEl.innerHTML = recentes.length
      ? recentes.map(s=>`<div class="acao-row" onclick="abrirFicha('${escAttr(String(s.id))}')">
          <span class="acao-mat">${sanitize(s.dataCadastro||'')}</span>
          <span class="acao-nome">${sanitize(s.nome)}</span>
          <span class="acao-tags"><span class="acao-tag info">${sanitize(s.tipoSocio||s.tipo||'-')}</span></span>
        </div>`).join('')
      : '<p style="color:#9ca3af;font-size:13px;padding:8px">Sem cadastros nos últimos 30 dias.</p>';
  }

  _bar('chart-completude',[
    {l:'Sem CPF',v:semCPF},
  ...
  _bar('chart-deps',[
    {l:'Regulares (Art. 22)', v:depsRegulares},
    {l:'Pendente comprovação', v:depsPendentes},
    {l:'Irregulares (fora do Art. 22)', v:depsIrregulares},
  ].filter(d=>d.v>0));
  // Top motivos de irregularidade
  const elMot = document.getElementById('chart-deps-motivos');
  if (elMot) {
    const top = Object.entries(motivosCount).sort((a,b)=>b[1]-a[1]).slice(0,6);
    if (top.length) {
      _bar('chart-deps-motivos', top.map(([l,v])=>({l,v})));
    } else {
      elMot.innerHTML = '<p style="color:#9ca3af;font-size:12px;padding:8px">Nenhuma irregularidade detectada nos dependentes.</p>';
    }
  }
}

// ---------- TABELA PRESIDENTES ORDENÁVEL ----------
function renderTabelaPresidentes(stats){
  const el = document.getElementById('tabela-presidentes'); if(!el) return;
  if(!stats.length){ el.innerHTML = '<p style="color:#9ca3af;font-size:13px">Sem presidentes registrados.</p>'; return; }
  const {col,dir} = presTableSort;
  stats.sort((a,b)=> dir==='asc' ? (a[col]>b[col]?1:-1) : (a[col]<b[col]?1:-1));
  const arrow = c => c===col ? (dir==='asc'?' ▲':' ▼') : '';
  el.innerHTML = `<div style="overflow-x:auto"><table class="pres-table">
    <thead><tr>
      <th onclick="sortPres('nome')">Presidente${arrow('nome')}</th>
      <th onclick="sortPres('total')" style="text-align:right">Total${arrow('total')}</th>
      <th onclick="sortPres('props')" style="text-align:right">Proprietários${arrow('props')}</th>
      <th onclick="sortPres('remidos')" style="text-align:right">Remidos${arrow('remidos')}</th>
      <th style="text-align:right">% Prop.</th>
      <th onclick="sortPres('deps')" style="text-align:right">Dependentes${arrow('deps')}</th>
      <th onclick="sortPres('depsIrr')" style="text-align:right;background:#fef2f2" title="Dependentes que violam o Art. 22">⚠ Irregulares${arrow('depsIrr')}</th>
    </tr></thead>
    <tbody>${stats.map(p=>`<tr>
      <td>
        <span class="pres-name" onclick="mostrarSociosPorPresidente('${escAttr(p.nome)}',null)">${sanitize(p.nome)}</span>
        ${(p._unificou && p._unificou.length) ? `<div style="font-size:10px;color:#7c3aed;margin-top:2px" title="Estas grafias estão sendo agrupadas automaticamente nesta gestão">↪ unificado: ${p._unificou.map(sanitize).join(', ')} <button onclick="aplicarUnificacaoPresidente('${escAttr(p.nome)}')" style="margin-left:4px;font-size:10px;padding:2px 6px;background:#7c3aed;color:#fff;border:0;border-radius:3px;cursor:pointer">Salvar grafia</button></div>` : ''}
      </td>
      <td style="text-align:right;font-weight:700">${p.total}</td>
      <td style="text-align:right">${p.props}</td>
      <td style="text-align:right">${p.remidos}</td>
      <td style="text-align:right;color:#7c3aed;font-weight:600">${p.total?Math.round(p.props/p.total*100):0}%</td>
      <td style="text-align:right">${p.deps||0}</td>
      <td style="text-align:right;font-weight:700;color:${(p.depsIrr||0)>0?'#dc2626':'#16a34a'};background:${(p.depsIrr||0)>0?'#fef2f2':'transparent'}">${p.depsIrr||0}${p.deps?` <span style="font-size:10px;color:#7f1d1d">(${Math.round((p.depsIrr||0)/p.deps*100)}%)</span>`:''}</td>
    </tr>`).join('')}</tbody>
  </table></div>`;
}
// Persiste a unificação: regrava todos os sócios cujo presidente bate com a chave para a grafia canônica
async function aplicarUnificacaoPresidente(nomeCanonico){
  const k = normPresidenteKey(nomeCanonico);
  const alvos = (socios||[]).filter(s => s.presidente && normPresidenteKey(s.presidente)===k && s.presidente!==nomeCanonico);
  if(!alvos.length){ alert('Nenhuma grafia divergente encontrada.'); return; }
  if(!confirm(`Deseja gravar "${nomeCanonico}" em ${alvos.length} sócio(s) que estavam com grafia diferente?`)) return;
  for(const s of alvos){
    s.presidente = nomeCanonico;
    try { await editarSocioNoServidor(s); } catch(e){ console.warn(e); }
  }
  try { localStorage.setItem(LOCAL_KEY, JSON.stringify(socios)); } catch(e){}
  atualizarListaPresidentes();
  renderDashboard();
  alert(`✓ ${alvos.length} sócio(s) atualizado(s) para a grafia "${nomeCanonico}".`);
}
function sortPres(col){
  if(presTableSort.col===col) presTableSort.dir = presTableSort.dir==='asc'?'desc':'asc';
  else { presTableSort.col=col; presTableSort.dir = col==='nome'?'asc':'desc'; }
  renderDashboard();
}

// ---------- COMPARATIVO DE GESTÕES ----------
function renderComparativo(){
  const out = document.getElementById('comparativo-out'); if(!out) return;
  const a = document.getElementById('cmp-pres-a')?.value;
  const b = document.getElementById('cmp-pres-b')?.value;
  if(!a || !b){ out.innerHTML = '<p style="color:#9ca3af;font-size:13px">Selecione dois presidentes para comparar.</p>'; return; }
  const calc = (nome) => {
    const k = normPresidenteKey(nome);
    const list = socios.filter(s => normPresidenteKey(s.presidente||'') === k);
    const props = list.filter(s=>getTipoNorm(s)==='proprietario').length;
    const remidos = list.filter(s=>getTipoNorm(s)==='remido').length;
    const ativos = list.filter(s=>getSitNorm(s)==='ativo').length;
    const idades = list.map(s=>_idadeDe(s.dataNasc)).filter(i=>i!==null);
    const idadeMedia = idades.length?Math.round(idades.reduce((x,y)=>x+y,0)/idades.length):null;
    const anos = list.map(s=>{const d=_dataRef(s);return d?d.getFullYear():null;}).filter(y=>y);
    const span = anos.length ? (Math.max(...anos)-Math.min(...anos)+1) : 1;
    let totalDeps=0, depsIrr=0;
    list.forEach(s=>(s.dependentes||[]).forEach(d=>{
      totalDeps++;
      if (avaliarDependente(d,s).status==='irregular') depsIrr++;
    }));
    return { total:list.length, props, remidos, ativos, idadeMedia, mediaAno:(list.length/span).toFixed(1), periodo: anos.length?`${Math.min(...anos)}–${Math.max(...anos)}`:'—', totalDeps, depsIrr };
  };
  const ca = calc(a), cb = calc(b);
  const row = (lbl,va,vb) => `<div class="cmp-metric"><span class="lbl">${lbl}</span><span class="val">${va}</span></div>`;
  const irrFmt = c => `${c.depsIrr} de ${c.totalDeps}${c.totalDeps?` (${Math.round(c.depsIrr/c.totalDeps*100)}%)`:''}`;
  out.innerHTML = `<div class="cmp-grid">
    <div class="cmp-col"><h4>${sanitize(a)}</h4>
      ${row('Período', ca.periodo, '')}
      ${row('Total de sócios', ca.total)}
      ${row('Proprietários', ca.props + ` (${ca.total?Math.round(ca.props/ca.total*100):0}%)`)}
      ${row('Remidos', ca.remidos)}
      ${row('Ativos hoje', ca.ativos)}
      ${row('Idade média', ca.idadeMedia?ca.idadeMedia+' anos':'—')}
      ${row('Cadastros/ano (média)', ca.mediaAno)}
      ${row('Dependentes cadastrados', ca.totalDeps)}
      ${row('⚠ Dep. irregulares (Art. 22)', `<span style="color:${ca.depsIrr?'#dc2626':'#16a34a'};font-weight:700">${irrFmt(ca)}</span>`)}
    </div>
    <div class="cmp-col"><h4>${sanitize(b)}</h4>
      ${row('Período', cb.periodo, '')}
      ${row('Total de sócios', cb.total)}
      ${row('Proprietários', cb.props + ` (${cb.total?Math.round(cb.props/cb.total*100):0}%)`)}
      ${row('Remidos', cb.remidos)}
      ${row('Ativos hoje', cb.ativos)}
      ${row('Idade média', cb.idadeMedia?cb.idadeMedia+' anos':'—')}
      ${row('Cadastros/ano (média)', cb.mediaAno)}
      ${row('Dependentes cadastrados', cb.totalDeps)}
      ${row('⚠ Dep. irregulares (Art. 22)', `<span style="color:${cb.depsIrr?'#dc2626':'#16a34a'};font-weight:700">${irrFmt(cb)}</span>`)}
    </div>
  </div>`;
}

// ---------- DRILL-DOWN POR PRESIDENTE ----------
function mostrarSociosPorPresidente(presidente, tipoFiltro) {
  const k = normPresidenteKey(presidente);
  let lista = socios.filter(s => normPresidenteKey(s.presidente||'') === k);
  const tipoLabel = tipoFiltro ? ` (${tipoFiltro})` : '';
  if (tipoFiltro) lista = lista.filter(s => getTipoNorm(s) === tipoFiltro);
...
function drillKpi(filtro){
  // Limpa filtros antes de aplicar (para o drill funcionar de forma previsível)
  const inp = document.getElementById('search-input');
  const ft  = document.getElementById('filter-tipo');
  const fs  = document.getElementById('filter-sit');
  if(inp) inp.value='';
  if(ft)  ft.value='';
  if(fs)  fs.value='';
  window._drillDepIrregular = false;

  if(!filtro){ showTab('socios'); renderSocios(); return; }

  // Filtro especial: sócios cujos dependentes são irregulares (Art. 22)
  if(filtro === '__dep_irregular__'){
    window._drillDepIrregular = true;
    showTab('socios');
    renderSocios();
    return;
  }
  ...
  showTab('socios');
  // Situações conhecidas vão no select de situação; tipos no select de tipo; resto vai no search
  const sits = ['ativo','suspenso','inadimplente','irregular','desligado'];
  if(sits.includes(filtro) && fs){ fs.value = filtro; }
  else if(filtro === 'proprietario' && ft){ ft.value = 'proprietario'; }
  else if(inp){ inp.value = filtro; }
  renderSocios();
}

// ---------- EXPORT CSV ----------
function exportarCSVDash(){
  const arr = _filtrar(socios);
  if(!arr.length){ alert('Nenhum sócio na seleção para exportar.'); return; }
  const cols = ['matricula','nome','cpf','tipoSocio','situacao','sexo','dataNasc','tel','telefone','email','endereco','bairro','cidade','presidente','dataCadastro'];
  const head = cols.join(';');
  const esc = v => { const s = String(v??'').replace(/"/g,'""'); return /[;"\n]/.test(s) ? `"${s}"` : s; };
  const body = arr.map(s => cols.map(c=>esc(s[c])).join(';')).join('\n');
  const csv = '\ufeff' + head + '\n' + body;
  const blob = new Blob([csv], {type:'text/csv;charset=utf-8'});
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url; a.download = `socios_${new Date().toISOString().slice(0,10)}.csv`;
...
function imprimirDashboard(){
  showTab('dashboard');
  // Para impressão, mostra todas as panes
  document.querySelectorAll('.dash-pane').forEach(p => p.style.display='block');
  renderDashboard();
  setTimeout(()=>{
    window.print();
    // restaura aba ativa
    setTimeout(()=>showDashPane(dashActivePane), 200);
  }, 300);
}
  ...
  v.innerHTML=`
    ...
    <div class="section"><div class="section-title">Dados do Sócio</div>
      <div class="ficha-grid">
        ...
        <div class="ficha-field"><div class="lbl">Situação</div><div class="val">${sitTag(s.situacao||s.status||'ativo')}</div></div>
        ${isIrregular(s)?'<div class="ficha-field"><div class="lbl">⚠️ Motivo da Irregularidade</div><div class="val" style="color:#c2410c;font-weight:600">'+sanitize(s.motivoIrregular||'(não informado)')+'</div></div>':''}
    ...
    ${(s.dependentes||[]).length>0?`<div class="section"><div class="section-title">Dependentes (${s.dependentes.length}) ${(()=>{const irr=s.dependentes.filter(d=>avaliarDependente(d,s).status==='irregular').length;return irr?'<span style="background:#fee2e2;color:#dc2626;padding:2px 8px;border-radius:10px;font-size:11px;font-weight:700">⚠ '+irr+' irregular(es)</span>':'';})()}</div>${s.dependentes.map((d,i)=>{const av=avaliarDependente(d,s);const docs=d.docs||{};const docsList=[docs.registroCivil&&'Reg. Civil',docs.tutela&&'Tutela',docs.declaracao2Socios&&'Decl. 2 sócios'].filter(Boolean).join(' · ');return '<div class="dep-card" style="margin-bottom:8px"><div style="display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap"><div><strong>'+sanitize(d.nome)+'</strong> · '+tipoDepLabel(d.tipoDep)+' · '+(d.dataNasc||'—')+' · '+(d.sexo==='M'?'M':'F')+'</div>'+statusDepBadge(av.status)+'</div>'+(av.motivos.length?'<div style="font-size:11px;color:#7f1d1d;margin-top:4px">⚠ '+av.motivos.map(m=>sanitize(m)).join(' · ')+'</div>':'')+(docsList?'<div style="font-size:11px;color:#475569;margin-top:4px">📄 '+docsList+'</div>':'')+(d.obsAuditoria?'<div style="font-size:11px;color:#475569;margin-top:4px;font-style:italic">Obs.: '+sanitize(d.obsAuditoria)+'</div>':'')+'</div>';}).join('')}</div>`:''}
  `;
}
  ...
  edit.innerHTML=`
    ...
    <div class="section"><div class="section-title">Dados Principais</div>
      <div class="grid2">
        ...
        <div class="field"><label>Data Nascimento</label><input id="e-datanasc" type="date" min="1900-01-01" max="${new Date().toISOString().split('T')[0]}" value="${s.dataNasc||''}" onchange="this.value=_sanitizeDataNasc(this.value,this)"/></div>
    ...
    <div class="section"><div class="section-title">Situação e Dados do Sócio</div>
      <div class="grid2">
        <div class="field"><label>Situação</label><select id="e-situacao" onchange="onSituacaoEditChange()"><option value="ativo" ${getSitNorm(s)==='ativo'?'selected':''}>Ativo</option><option value="suspenso" ${getSitNorm(s)==='suspenso'?'selected':''}>Suspenso</option><option value="inadimplente" ${getSitNorm(s)==='inadimplente'?'selected':''}>Inadimplente</option><option value="irregular" ${getSitNorm(s)==='irregular'?'selected':''}>Irregular</option><option value="desligado" ${getSitNorm(s)==='desligado'?'selected':''}>Desligado</option></select></div><div class="field col2" id="e-motivo-irregular-wrap" style="display:${getSitNorm(s)==='irregular'?'':'none'}"><label>Motivo da Irregularidade <span class="req">*</span></label><input id="e-motivo-irregular" type="text" maxlength="200" placeholder="Ex: sem RG, sem recibo de adesão..." value="${sanitize(s.motivoIrregular||'')}"/><div class="hint">Obrigatório quando a situação é Irregular.</div></div>
      ...
      <div class="grid2">
        <div class="field"><label>Presidente Responsável</label><input id="e-presidente" list="lista-presidentes" value="${s.presidente||''}" oninput="sugerirPresidente(this,'e-presidente-hint')" onblur="this.value=canonPresidente(this.value);sugerirPresidente(this,'e-presidente-hint')"/><div class="hint" id="e-presidente-hint" style="font-size:11px"></div></div>
        <div class="field"><label>Data do Acordo</label><input id="e-dataacordo" type="date" min="1900-01-01" value="${s.dataAcordo||''}" ${!s.dataAcordo?'disabled':''} onchange="_renderEditDepsList()"/>
          <label style="display:flex;align-items:center;gap:6px;font-size:12px;color:#475569;margin-top:4px;font-weight:normal;cursor:pointer">
            <input id="e-dataacordo-ni" type="checkbox" ${!s.dataAcordo?'checked':''} onchange="(function(cb){var i=document.getElementById('e-dataacordo');i.disabled=cb.checked;if(cb.checked)i.value='';_renderEditDepsList();})(this)"/>
            Data não informada na ficha
          </label>
        </div>
        ...
        <div class="field"><label>Forma de Aquisição</label>
          <select id="e-origem">
            <option value="">— não informado —</option>
            <option value="transferencia" ${s.origemTitulo==='transferencia'?'selected':''}>Transferência</option>
            <option value="heranca" ${s.origemTitulo==='heranca'?'selected':''}>Herança / Causa Mortis</option>
            <option value="adquirido_clube" ${s.origemTitulo==='adquirido_clube'?'selected':''}>Adquirido diretamente do Clube</option>
            <option value="familiar" ${s.origemTitulo==='familiar'?'selected':''}>Familiar (isento)</option>
            <option value="compra" ${s.origemTitulo==='compra'?'selected':''}>Compra/Venda</option>
            <option value="outro" ${s.origemTitulo==='outro'?'selected':''}>Outro</option>
          </select>
        </div>
        <div class="field"><label>Cedente (anterior)</label><input id="e-cedente" value="${s.cedente||''}"/></div>
      </div>
    </div>
...
async function salvarEdicao(){
  ...
  s.presidente  = canonPresidente(document.getElementById('e-presidente')?.value.trim() || s.presidente || '');
  if (document.getElementById('e-origem')) s.origemTitulo = document.getElementById('e-origem').value || s.origemTitulo || '';
  if (document.getElementById('e-cedente')) s.cedente = document.getElementById('e-cedente').value.trim();
  s.dataAcordo  = document.getElementById('e-dataacordo-ni')?.checked ? '' : (document.getElementById('e-dataacordo')?.value || '');
  ...
  s.motivoIrregular = (document.getElementById('e-motivo-irregular')?.value||'').trim();
  if (s.situacao === 'irregular' && !s.motivoIrregular) {
    const m=document.getElementById('ficha-msg');
    if(m){m.className='alert err';m.textContent='Quando a situação é Irregular, o Motivo da Irregularidade é obrigatório.';m.style.display='block';}
    document.getElementById('e-motivo-irregular')?.focus();
    return;
  }
  ...
  s.dependentes = editDepsData.map(d => ({
    nome:d.nome, dataNasc:d.dataNasc, sexo:d.sexo, tipoDep:d.tipoDep, foto:d.foto||'',
    inicioUniao:d.inicioUniao||'', estadoCivilDep:d.estadoCivilDep||'',
    incapaz:!!d.incapaz, dependenciaFinanceira:!!d.dependenciaFinanceira,
    obsAuditoria:d.obsAuditoria||'', docs:d.docs||{}
  }));
...
function tipoDepLabel(v) {
  const mapa = {
    // Tipos previstos no Art. 22
    filho: 'Filho(a) menor de 18 anos',
    filha_solteira: 'Filha solteira',
    conjuge: 'Cônjuge',
    companheiro: 'Companheiro(a) - união estável',
    pai_mae_60: 'Pai/Mãe (acima de 60 ou dep. financeira)',
    sogro: 'Sogro(a)',
    padrasto_madrasta: 'Padrasto/Madrasta',
    enteado: 'Enteado(a)',
    tutelado: 'Tutelado(a) / Guarda judicial',
    irmao_menor: 'Irmão(ã) menor de 18 anos',
    // Tipos NÃO previstos no Art. 22 (sempre irregulares)
    neto: 'Neto(a) (não previsto no Art. 22)',
    sobrinho: 'Sobrinho(a) (não previsto no Art. 22)',
    primo: 'Primo(a) (não previsto no Art. 22)',
    outro_irregular: 'Outro - fora do Art. 22'
  };
  return mapa[v] || v || 'Dependente';
}

// ===== AVALIAÇÃO DE DEPENDENTES (Art. 22 do Estatuto BTC) =====
// Retorna { status: 'regular' | 'pendente' | 'irregular', motivos: string[] }
function avaliarDependente(d, socio) {
  const motivos = [];
  if (!d || !d.tipoDep) {
    return { status: 'pendente', motivos: ['Tipo de dependente não informado'] };
  }
  const idade = _idadeDe(d.dataNasc);
  const docs = d.docs || {};
  const tiposForaArt22 = ['neto','sobrinho','primo','outro_irregular'];
  if (tiposForaArt22.includes(d.tipoDep)) {
    motivos.push('Parentesco não previsto no Art. 22 do Estatuto');
    return { status: 'irregular', motivos };
  }
  switch (d.tipoDep) {
    case 'conjuge':
      if (!docs.registroCivil) motivos.push('Falta certidão de casamento (Art. 22-e)');
      return { status: motivos.length ? 'pendente' : 'regular', motivos };
    case 'companheiro': {
      if (!d.inicioUniao) {
        motivos.push('Início da união estável não informado (Art. 22-b)');
        return { status: 'pendente', motivos };
      }
      const ini = new Date(d.inicioUniao);
      const anos = (Date.now() - ini.getTime()) / (1000*60*60*24*365.25);
      if (anos < 2) motivos.push(`União estável com ${anos.toFixed(1)} ano(s) — exige mais de 2 anos (Art. 22-b)`);
      if (!docs.declaracao2Socios) motivos.push('Falta declaração de 2 sócios (Art. 22-e)');
      if (motivos.some(m => m.includes('exige mais de 2'))) return { status: 'irregular', motivos };
      return { status: motivos.length ? 'pendente' : 'regular', motivos };
    }
    case 'filho':
    case 'enteado':
    case 'tutelado': {
      // Masculino: até 18 OU incapaz; Feminino: solteira E (menor de 18 OU com dependência financeira/guarda judicial)
      if (d.sexo === 'F') {
        const ec = d.estadoCivilDep || 'solteira';
        if (ec !== 'solteira') {
          motivos.push(`Filha/enteada com estado civil "${ec}" — Art. 22-d exige solteira`);
          return { status: 'irregular', motivos };
        }
        if (idade !== null && idade >= 18) {
          const temGuarda = !!(d.docs && d.docs.tutela);
          const temDepFin = !!d.dependenciaFinanceira;
          if (!temGuarda && !temDepFin && !d.incapaz) {
            motivos.push(`Filha/enteada/irmã solteira com ${idade} anos — exige comprovação de dependência financeira ou guarda judicial (Art. 22-d)`);
            return { status: 'irregular', motivos };
          }
        }
        return { status: motivos.length ? 'pendente' : 'regular', motivos };
      ...
      // Masculino
      if (idade === null) {
        motivos.push('Data de nascimento não informada');
        return { status: 'pendente', motivos };
      }
      if (idade > 18 && !d.incapaz) {
        motivos.push(`${d.sexo==='M'?'Filho/enteado':'Dep.'} com ${idade} anos sem laudo de incapacidade — Art. 22-c`);
        return { status: 'irregular', motivos };
      }
      return { status: motivos.length ? 'pendente' : 'regular', motivos };
    }
    case 'filha_solteira': {
      const ec = d.estadoCivilDep || 'solteira';
      if (ec !== 'solteira') {
        motivos.push(`Filha com estado civil "${ec}" — Art. 22-d exige solteira`);
        return { status: 'irregular', motivos };
      }
      if (idade !== null && idade >= 18) {
        const temGuarda = !!(d.docs && d.docs.tutela);
        const temDepFin = !!d.dependenciaFinanceira;
        if (!temGuarda && !temDepFin && !d.incapaz) {
          motivos.push(`Filha solteira com ${idade} anos — exige comprovação de dependência financeira ou guarda judicial (Art. 22-d)`);
          return { status: 'irregular', motivos };
        }
      }
      return { status: motivos.length ? 'pendente' : 'regular', motivos };
    ...
    case 'padrasto_madrasta': {
      ...
      if (idade < 60 && !d.dependenciaFinanceira) {
        motivos.push(`${tipoDepLabel(d.tipoDep)} com ${idade} anos sem comprovação de dependência financeira — Art. 22-f`);
        return { status: 'irregular', motivos };
      }
      return { status: 'regular', motivos };
    }
    case 'irmao_menor': {
      if (idade === null) {
        motivos.push('Data de nascimento não informada');
      ...
      // Art. 22-g: idade < 18 NA DATA DA ADMISSÃO
      const refAdmissao = socio && socio.dataAcordo ? new Date(socio.dataAcordo) : null;
      if (refAdmissao && d.dataNasc) {
        const nasc = new Date(d.dataNasc);
        const idadeNaAdmissao = (refAdmissao.getTime() - nasc.getTime()) / (1000*60*60*24*365.25);
        if (idadeNaAdmissao >= 18) {
          motivos.push(`Irmão tinha ${idadeNaAdmissao.toFixed(0)} anos na data do acordo — Art. 22-g exige < 18`);
          return { status: 'irregular', motivos };
        }
        return { status: 'regular', motivos };
      }
      // Sem data do acordo: usa idade atual como proxy + alerta
      if (idade >= 18) {
        motivos.push(`Irmão com ${idade} anos hoje — sem data do acordo do sócio para validar Art. 22-g`);
        return { status: 'pendente', motivos };
      }
      return { status: 'regular', motivos };
    }
    case 'enteado':
      return { status: 'pendente', motivos: ['Necessita comprovação de guarda/tutela'] };
    default:
      return { status: 'pendente', motivos: ['Tipo de dependente não classificado'] };
  }
}

function statusDepBadge(status) {
  const map = {
    regular:    { cor: '#16a34a', bg: '#dcfce7', txt: 'Regular',    icon: '✓' },
    pendente:   { cor: '#b45309', bg: '#fef3c7', txt: 'Pendente',   icon: '!' },
    irregular:  { cor: '#dc2626', bg: '#fee2e2', txt: 'Irregular',  icon: '✗' }
  };
  const m = map[status] || map.pendente;
  return `<span class="dep-status-badge" style="display:inline-flex;align-items:center;gap:4px;background:${m.bg};color:${m.cor};padding:3px 8px;border-radius:12px;font-size:11px;font-weight:700">${m.icon} ${m.txt}</span>`;
}
...
function exportarCSV(filtro) {
  ...
  const cols = ['matricula','nome','cpf','rg','dataNasc','sexo','estadoCivil','telefone','email',
    'cep','bairro','endereco','tipoSocio','situacao','motivoIrregular','atividade','presidente','dataCadastro','obs'];
...
async function gerarPdfCarteirinha(matricula, isDependente, idxDep){
  ...
  if(isIrregular(s)){
    alert('⛔ Não é possível emitir carteirinha: este sócio está com situação IRREGULAR.\n\nMotivo registrado: '+(s.motivoIrregular||'(não informado)')+'\n\nRegularize a documentação antes de emitir a carteirinha.');
    return;
  }
...
const REL_COLUNAS = [
  ...
  {k:'_depsIrregulares', l:'Nº Dep. Irregulares (Art. 22)'},
  {k:'_motivosIrregulares', l:'Motivos das Irregularidades'},
];
...
function abrirModalRelatorio(){
  ...
  if(!modal){
    ...
    // popula presidentes únicos (agrupados por chave normalizada)
    const presMap = new Map();
    socios.forEach(s=>{ const p=(s.presidente||'').trim(); if(!p) return; const k=normPresidenteKey(p); if(!presMap.has(k)) presMap.set(k,p); });
    const presSel = document.getElementById('rel-pres');
    [...presMap.values()].sort().forEach(p=>{
...
function relFiltrarSocios(){
  ...
  return socios.filter(s=>{
    ...
    if(pres && normPresidenteKey(s.presidente||'') !== normPresidenteKey(pres)) return false;
...
function _valorParaRel(s, k){
  ...
  if(k === '_depsIrregulares') return (s.dependentes||[]).filter(d=>avaliarDependente(d,s).status==='irregular').length;
  if(k === '_motivosIrregulares') {
    const out = [];
    (s.dependentes||[]).forEach(d=>{
      const av = avaliarDependente(d,s);
      if (av.status==='irregular') out.push((d.nome||'(sem nome)')+': '+av.motivos.join('; '));
    });
    return out.join(' | ');
  }
...
/* ============================================================
   ===== v5: IMPRESSAO EM LOTE DE CARTEIRINHAS =====
   - Seletor de layout: 8/9/10/12/16 (rascunho) por folha A4
   - Layout 9/folha (3x3 retrato) padrao - replica modelo .docx
   - Marca d'agua por situacao: Suspenso (cinza) / Inadimplente (vermelho)
   - Sócios IRREGULARES sao bloqueados automaticamente
   - Prévia em modal antes de gerar PDF
   - Preferencia de layout salva em localStorage
   ============================================================ */

// Configuracoes de layout disponíveis
const CART_LAYOUTS = {
  '9':  { label: '9 por folha (3x3 retrato) - recomendado', orient: 'portrait',  cols: 3, rows: 3, cardW: 63,  cardH: 85, pageW: 210, pageH: 297, vertical: true,  containerPx: 360 },
  '8':  { label: '8 por folha (2x4 paisagem) - CR80 padrao', orient: 'landscape', cols: 2, rows: 4, cardW: 85.6,cardH: 54, pageW: 297, pageH: 210, vertical: false, containerPx: 420 },
  '10': { label: '10 por folha (2x5 paisagem) - fino',       orient: 'landscape', cols: 2, rows: 5, cardW: 85.6,cardH: 40, pageW: 297, pageH: 210, vertical: false, containerPx: 420 },
  '12': { label: '12 por folha (3x4 retrato) - crachá',      orient: 'portrait',  cols: 3, rows: 4, cardW: 63,  cardH: 67, pageW: 210, pageH: 297, vertical: true,  containerPx: 320 },
  '16': { label: '16 por folha (4x4 rascunho)',              orient: 'portrait',  cols: 4, rows: 4, cardW: 47,  cardH: 60, pageW: 210, pageH: 297, vertical: true,  containerPx: 280 },
};
const CART_LAYOUT_DEFAULT = '9';
function _getCartLayoutPref(){ try { return localStorage.getItem('cart_layout_pref') || CART_LAYOUT_DEFAULT; } catch(e){ return CART_LAYOUT_DEFAULT; } }
function _setCartLayoutPref(v){ try { localStorage.setItem('cart_layout_pref', v); } catch(e){} }

// ---- Marca d'agua por situacao ----
function _watermarkOverlay(situacao){
  const sit = (situacao||'').toLowerCase();
  let texto = '', cor = '';
  if (sit === 'suspenso')      { texto = 'SUSPENSO';      cor = 'rgba(75,85,99,0.55)';   } // cinza
  else if (sit === 'inadimplente'){ texto = 'INADIMPLENTE'; cor = 'rgba(220,38,38,0.45)'; } // vermelho
  else return '';
  return '<div style="position:absolute;inset:0;pointer-events:none;display:flex;align-items:center;justify-content:center;overflow:hidden;z-index:50">'
       + '<div style="transform:rotate(-22deg);font-family:Arial,sans-serif;font-weight:900;font-size:42px;letter-spacing:6px;color:'+cor+';text-shadow:0 0 2px rgba(255,255,255,.6);white-space:nowrap">'
       + texto + '</div></div>';
}

// ---- Template VERTICAL (replica modelo .docx) usado nos layouts 9/12/16 ----
function btcCardVerticalHTML(pessoa, matricula, isDependent, tipoSocio, situacao){
  const nome = isDependent ? (pessoa.nome+' (Dependente)') : pessoa.nome;
  const fotoHtml = pessoa.foto
    ? '<img src="'+pessoa.foto+'" style="width:100%;height:100%;object-fit:cover"/>'
    : '<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;background:#e5e7eb;color:#94a3b8;font-size:11px">FOTO</div>';
  let stripeText = 'SOCIO CONTRIBUINTE';
  let stripeBg   = '#0f2a49';
  if (tipoSocio==='proprietario'){ stripeText='SOCIO PROPRIETARIO'; stripeBg='#0f2a49'; }
  else if (tipoSocio==='nao_socio'){ stripeText='NAO SOCIO';        stripeBg='#6b7280'; }
  else if (tipoSocio==='remido'){ stripeText='SOCIO REMIDO';        stripeBg='#7c2d12'; }
  else if (tipoSocio==='benemerito'){ stripeText='SOCIO BENEMERITO'; stripeBg='#854d0e'; }
  const matriculaDisplay = (tipoSocio==='nao_socio') ? '---' : matricula;
  const atividade = pessoa.atividade || '---';
  const wm = _watermarkOverlay(situacao);

  return '<div class="btc-card-v" style="position:relative;width:100%;aspect-ratio:63/85;background:#fff;border:1px solid #0f2a49;border-radius:6px;overflow:hidden;font-family:Arial,Helvetica,sans-serif;color:#0f172a;display:flex;flex-direction:column;box-sizing:border-box">'
    +   wm
    // Cabeçalho
    +   '<div style="background:#0f2a49;color:#fff;padding:6px 8px;text-align:center;font-size:9px;line-height:1.15;font-weight:700">'
    +     '<div style="font-size:11px">CLUBE BANCO DO BRASIL</div>'
    +     '<div style="font-weight:400;opacity:.85">CARTEIRINHA DE SOCIO</div>'
    +   '</div>'
    // Faixa categoria
    +   '<div style="background:'+stripeBg+';color:#fff;text-align:center;font-size:9px;font-weight:700;padding:3px 4px;letter-spacing:.5px">'+stripeText+'</div>'
    // Foto
    +   '<div style="width:60%;aspect-ratio:1/1;margin:8px auto 4px;border:1px solid #0f2a49;overflow:hidden;background:#f1f5f9">'
    +     fotoHtml
    +   '</div>'
    // Dados
    +   '<div style="flex:1;padding:4px 8px;font-size:8.5px;line-height:1.3">'
    +     '<div style="margin-bottom:2px"><strong>Nome:</strong><br>'+sanitize(nome)+'</div>'
    +     '<div style="margin-bottom:2px"><strong>Matricula:</strong> '+sanitize(String(matriculaDisplay))+'</div>'
    +     '<div><strong>Atividade:</strong> '+sanitize(String(atividade))+'</div>'
    +   '</div>'
    // Rodape
    +   '<div style="border-top:1px solid #94a3b8;text-align:center;padding:3px 4px;font-size:7.5px;font-weight:700;color:#0f2a49;letter-spacing:.5px">PRESIDENTE ADMINISTRATIVO</div>'
    + '</div>';
}

// ---- Card horizontal com marca d'agua (wrapper sobre btcCardHTML existente) ----
function btcCardHorizontalComWM(pessoa, matricula, isDependent, tipoSocio, situacao){
  const wm = _watermarkOverlay(situacao);
  const cardHtml = btcCardHTML(pessoa, matricula, isDependent, tipoSocio);
  if (!wm) return cardHtml;
  // injeta a marca d'agua dentro do primeiro <div class="btc-card">
  return cardHtml.replace('<div class="btc-card">', '<div class="btc-card" style="position:relative">'+wm);
}

// ---- Botao "Carteirinhas em Lote" injetado na barra da aba Socios ----
...
if (_origRenderSocios) {
  ...
  _wrapped._pag = _origRenderSocios._pag;
  window.renderSocios = _wrapped;
}
...
function abrirModalCarteirinhasLote() {
  ...
  const _gestoesMap = new Map();
  (socios||[]).forEach(s=>{ const p=(s.presidente||'').trim(); if(!p) return; const k=normPresidenteKey(p); if(!_gestoesMap.has(k)) _gestoesMap.set(k,p); });
  const gestoes = Array.from(_gestoesMap.values()).sort();
  const bairros = Array.from(new Set((socios||[]).map(s => (s.bairro||'').trim()).filter(Boolean))).sort();
  const layoutPref = _getCartLayoutPref();
  const layoutOpts = Object.entries(CART_LAYOUTS).map(([k,c]) =>
    '<option value="'+k+'"'+(k===layoutPref?' selected':'')+'>'+c.label+'</option>'
  ).join('');
  ...
  <div id="modal-cart-lote" style="position:fixed;inset:0;background:rgba(0,0,0,.55);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px">
    <div style="background:#fff;border-radius:10px;max-width:760px;width:100%;max-height:92vh;overflow:auto;box-shadow:0 20px 60px rgba(0,0,0,.3)">
      ...
      <div style="padding:18px 20px">
        <div style="background:#fef3c7;border-left:4px solid #f59e0b;padding:8px 12px;margin-bottom:14px;font-size:12px;color:#78350f;border-radius:4px">
          ℹ️ <strong>Sócios "Irregulares" são bloqueados automaticamente</strong> e não geram carteirinha. Sócios <strong>Suspensos</strong> e <strong>Inadimplentes</strong> recebem marca d'água indicando a situação.
        </div>

        <div style="margin-bottom:14px">
          <label style="display:block;font-size:12px;font-weight:600;color:#374151;margin-bottom:4px">📐 Layout de impressão</label>
          <select id="cl-layout" style="width:100%;padding:8px;border:2px solid #0f2a49;border-radius:6px;font-weight:600;background:#f8fafc">
            ${layoutOpts}
          </select>
          <div style="font-size:11px;color:#64748b;margin-top:4px">Sua escolha será lembrada nas próximas impressões.</div>
        </div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
          ...
          <div>
            ...
            <select id="cl-sit" style="width:100%;padding:8px;border:1px solid #d1d5db;border-radius:6px">
              <option value="">Todas (exceto Irregular)</option>
              <option value="ativo" selected>Ativo</option>
              <option value="suspenso">Suspenso (com marca d'água)</option>
              <option value="inadimplente">Inadimplente (com marca d'água)</option>
              <option value="desligado">Desligado</option>
            </select>
          </div>
          <div>
        ...
        <div style="background:#f1f5f9;padding:10px 12px;border-radius:6px;margin-top:14px;font-size:13px">
          <strong>Resumo:</strong> <span id="cl-preview-count">—</span> carteirinha(s) ·
          <span id="cl-preview-paginas">—</span> folha(s) A4
          <button onclick="clAtualizarPreview()" style="background:#3b82f6;color:#fff;border:0;padding:4px 10px;border-radius:4px;cursor:pointer;float:right;font-size:12px">🔄 Atualizar</button>
        </div>
      </div>
      <div style="padding:14px 20px;border-top:1px solid #e5e7eb;display:flex;justify-content:flex-end;gap:10px;background:#f8fafc">
        <button onclick="document.getElementById('modal-cart-lote').remove()" style="background:#fff;color:#374151;border:1px solid #d1d5db;padding:10px 16px;border-radius:6px;cursor:pointer">Cancelar</button>
        <button onclick="visualizarPreviaLote()" style="background:#0891b2;color:#fff;border:0;padding:10px 18px;border-radius:6px;cursor:pointer;font-weight:600">👁️ Visualizar prévia</button>
  ...
  ['cl-cat','cl-sit','cl-gestao','cl-bairro','cl-busca','cl-deps','cl-layout'].forEach(id=>{
    const el=document.getElementById(id);
    if(!el) return;
    el.addEventListener('change', () => { if(id==='cl-layout') _setCartLayoutPref(el.value); clAtualizarPreview(); });
    if(el.tagName==='INPUT' && el.type==='text') el.addEventListener('input', clAtualizarPreview);
  });
}

function _layoutAtual(){
  const v = document.getElementById('cl-layout')?.value || _getCartLayoutPref();
  return CART_LAYOUTS[v] || CART_LAYOUTS[CART_LAYOUT_DEFAULT];
}

// ---- Coleta com bloqueio automatico de Irregular ----
function _coletarPessoasLote() {
  ...
  const filtrados = (socios || []).filter(s => {
    // BLOQUEIO: nunca incluir Irregular
    if (isIrregular(s)) return false;
    if (cat && getTipoNorm(s) !== cat) return false;
    if (sit && getSitNorm(s).toLowerCase() !== sit) return false;
    if (gestao && normPresidenteKey(s.presidente||'') !== normPresidenteKey(gestao)) return false;
  ...
  const pessoas = [];
  filtrados.forEach(s => {
    pessoas.push({ pessoa: s, matricula: s.matricula, isDep: false, tipoSocio: getTipoNorm(s), situacao: getSitNorm(s) });
...
function clAtualizarPreview() {
  const elC = document.getElementById('cl-preview-count');
  const elP = document.getElementById('cl-preview-paginas');
  if (!elC) return;
  const n = _coletarPessoasLote().length;
  const lay = _layoutAtual();
  const porFolha = lay.cols * lay.rows;
  elC.textContent = n;
  if (elP) elP.textContent = Math.ceil(n / porFolha);
}

// ---- Renderiza HTML do card conforme layout ----
function _renderCardParaLayout(p, layout){
  if (layout.vertical) {
    return btcCardVerticalHTML(p.pessoa, p.matricula, p.isDep, p.tipoSocio, p.situacao);
  }
  return btcCardHorizontalComWM(p.pessoa, p.matricula, p.isDep, p.tipoSocio, p.situacao);
}

// ---- Visualizar prévia (modal com primeira pagina) ----
async function visualizarPreviaLote(){
  if (typeof html2canvas === 'undefined') { alert('Biblioteca html2canvas nao carregada.'); return; }
  const pessoas = _coletarPessoasLote();
  if (!pessoas.length) { alert('Nenhuma carteirinha atende aos filtros.'); return; }
  const lay = _layoutAtual();
  const porFolha = lay.cols * lay.rows;
  const primPag = pessoas.slice(0, porFolha);

  const btn = document.querySelector('#modal-cart-lote button[onclick^="visualizarPreviaLote"]');
  if (btn) { btn.disabled = true; btn.textContent = '⏳ Renderizando…'; }

  try {
    // Monta um container que simula a folha A4 (proporcionalmente)
    const escala = 2.6; // px por mm para visualizacao razoável
    const folhaW = lay.pageW * escala, folhaH = lay.pageH * escala;
    const cardWpx = lay.cardW * escala, cardHpx = lay.cardH * escala;
    const gridWpx = lay.cols * cardWpx, gridHpx = lay.rows * cardHpx;
    const mX = (folhaW - gridWpx) / 2, mY = (folhaH - gridHpx) / 2;

    let cellsHtml = '';
    primPag.forEach((p, i) => {
      const col = i % lay.cols, row = Math.floor(i / lay.cols);
      const x = mX + col * cardWpx, y = mY + row * cardHpx;
      cellsHtml += '<div style="position:absolute;left:'+x+'px;top:'+y+'px;width:'+cardWpx+'px;height:'+cardHpx+'px;overflow:hidden">'
                + _renderCardParaLayout(p, lay) + '</div>';
    });

    const previaModal = document.createElement('div');
    previaModal.id = 'modal-cart-previa';
    previaModal.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,.75);z-index:10000;display:flex;align-items:center;justify-content:center;padding:20px;overflow:auto';
    previaModal.innerHTML = ''
      + '<div style="background:#fff;border-radius:10px;max-width:95vw;max-height:95vh;overflow:auto;box-shadow:0 20px 60px rgba(0,0,0,.5)">'
      +   '<div style="padding:14px 20px;border-bottom:1px solid #e5e7eb;display:flex;justify-content:space-between;align-items:center;background:#0891b2;color:#fff;border-radius:10px 10px 0 0">'
      +     '<h3 style="margin:0;font-size:16px">👁️ Prévia da 1ª folha — '+lay.label+' · página 1 de '+Math.ceil(pessoas.length/porFolha)+'</h3>'
      +     '<button onclick="document.getElementById(\'modal-cart-previa\').remove()" style="background:#ef4444;color:#fff;border:0;border-radius:6px;width:32px;height:32px;font-size:18px;cursor:pointer">×</button>'
      +   '</div>'
      +   '<div style="padding:20px;background:#475569">'
      +     '<div style="position:relative;width:'+folhaW+'px;height:'+folhaH+'px;background:#fff;box-shadow:0 4px 16px rgba(0,0,0,.3);margin:0 auto">'
      +       cellsHtml
      +     '</div>'
      +   '</div>'
      +   '<div style="padding:12px 20px;border-top:1px solid #e5e7eb;background:#f8fafc;display:flex;justify-content:space-between;align-items:center;gap:10px">'
      +     '<div style="font-size:12px;color:#64748b">Total: <strong>'+pessoas.length+' carteirinha(s)</strong> em <strong>'+Math.ceil(pessoas.length/porFolha)+' folha(s) A4</strong></div>'
      +     '<div>'
      +       '<button onclick="document.getElementById(\'modal-cart-previa\').remove()" style="background:#fff;color:#374151;border:1px solid #d1d5db;padding:8px 14px;border-radius:6px;cursor:pointer;margin-right:8px">Fechar</button>'
      +       '<button onclick="document.getElementById(\'modal-cart-previa\').remove();gerarPdfCarteirinhasLote()" style="background:#0f2a49;color:#fff;border:0;padding:8px 16px;border-radius:6px;cursor:pointer;font-weight:600">🖨️ Gerar PDF agora</button>'
      +     '</div>'
      +   '</div>'
      + '</div>';
    document.body.appendChild(previaModal);
  } catch(e){
    console.error(e);
    alert('Erro ao gerar prévia: ' + e.message);
  } finally {
    if (btn) { btn.disabled = false; btn.textContent = '👁️ Visualizar prévia'; }
  }
}
...
async function gerarPdfCarteirinhasLote() {
  if (typeof window.jspdf === 'undefined' || typeof html2canvas === 'undefined') {
    alert('Bibliotecas jsPDF/html2canvas nao carregadas.');
    return;
  }
  const pessoas = _coletarPessoasLote();
  if (!pessoas.length) { alert('Nenhuma carteirinha atende aos filtros (sócios Irregulares são bloqueados automaticamente).'); return; }
  ...
  try {
    const { jsPDF } = window.jspdf;
    const pdf = new jsPDF({ orientation: lay.orient, unit: 'mm', format: 'a4' });
    const pageW = lay.pageW, pageH = lay.pageH;
    const cardW = lay.cardW, cardH = lay.cardH;
    ...
    for (let i = 0; i < pessoas.length; i++) {
      const slot = i % porFolha;
      if (slot === 0 && i > 0) pdf.addPage();

      const p = pessoas[i];
      const tmp = document.createElement('div');
      // Container respeitando proporcao do card
      const containerW = lay.containerPx;
      const containerH = Math.round(containerW * (cardH / cardW));
      tmp.style.cssText = 'position:fixed;left:-9999px;top:0;background:#fff;width:'+containerW+'px;height:'+containerH+'px;overflow:hidden';
      tmp.innerHTML = _renderCardParaLayout(p, lay);
      document.body.appendChild(tmp);
      let img;
      try {
        const canvas = await html2canvas(tmp, { scale: 2, backgroundColor: '#ffffff', useCORS: true, width: containerW, height: containerH });
    ...
    const nomeArq = 'Carteirinhas_Lote_' + ts + '_' + pessoas.length + 'un_' + porFolha + 'pf.pdf';
    pdf.save(nomeArq);
    registrarAuditoria('CARTEIRINHA_LOTE_PDF', pessoas.length + ' carteirinhas em ' + Math.ceil(pessoas.length/porFolha) + ' folhas A4 (layout '+porFolha+'/folha)');
