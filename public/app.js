...
async function loadData() {
  ...
  // Fallback da planilha removido - dados vêm apenas do servidor/localStorage
...
function restaurarPlanilha() {
  alert('Funcionalidade desativada. Os dados da planilha foram removidos para evitar duplicidade e bugs. Os sócios agora são gerenciados apenas pelo servidor e cadastro manual.');
}
...
async function salvarSocio() {
  ...
  // Validação básica (sempre obrigatórios)
  if (!nome) { showMsg('err', '\u274C Nome \u00e9 obrigat\u00f3rio.'); return; }
  if (cpf && !validarCPF(cpf)) { showMsg('err', '\u274C CPF inv\u00e1lido. Verifique os d\u00edgitos.'); return; }
  ...
  if (isNaoSocio) {
    ...
    if (socios.find(s => String(s.matricula) === mat)) {
      showMsg('err', '\u274C J\u00e1 existe um s\u00f3cio com esta matr\u00edcula.');
      return;
    }
  }
...
function renderDashboard(){
  ...
  // === FILTRO POR PRESIDENTE — Lista de sócios por gestão ===
  const presFilterEl = document.getElementById('dash-pres-filter');
  if (!presFilterEl) {
    // Cria a seção dinamicamente após o último gráfico de presidentes
    const section = document.createElement('div');
    section.className = 'dash-card';
    section.id = 'dash-pres-filter';
    section.innerHTML = `
      <div class="dash-card-title">📋 Sócios por Gestão do Presidente</div>
      <div style="margin-bottom:12px">
        <select id="pres-select" onchange="filtrarPorPresidente()" style="width:100%;padding:10px 12px;border:1px solid #d1d5db;border-radius:8px;font-size:14px;background:#fff">
          <option value="">Selecione um presidente...</option>
          ${Object.keys(presCount).sort().map(p => '<option value="'+p+'">'+p+' ('+presCount[p]+' sócios)</option>').join('')}
        </select>
      </div>
      <div id="pres-socios-list"></div>
    `;
    // Insere após chart-pres-total
    const presTotal = document.getElementById('chart-pres-total');
    if (presTotal) {
      const parent = presTotal.closest('.dash-card');
    ...
    const sel = document.getElementById('pres-select');
    if (sel) {
      const cur = sel.value;
      sel.innerHTML = '<option value="">Selecione um presidente...</option>' +
        Object.keys(presCount).sort().map(p => '<option value="'+p+'"'+(p===cur?' selected':'')+'>'+p+' ('+presCount[p]+' sócios)</option>').join('');
      if (cur) filtrarPorPresidente();
    }
  }
...
function filtrarPorPresidente() {
  const pres = document.getElementById('pres-select')?.value || '';
  const listEl = document.getElementById('pres-socios-list');
  if (!listEl) return;
  if (!pres) { listEl.innerHTML = '<p style="color:#9ca3af;font-size:13px">Selecione um presidente para ver os sócios da gestão.</p>'; return; }
  
  const filtrados = socios.filter(s => s.presidente === pres);
  if (!filtrados.length) { listEl.innerHTML = '<p style="color:#9ca3af;font-size:13px">Nenhum sócio encontrado para este presidente.</p>'; return; }
  
  // Ordena por matrícula
  filtrados.sort((a, b) => Number(a.matricula || 0) - Number(b.matricula || 0));
  
  listEl.innerHTML = `
    <div style="margin-bottom:8px;font-size:13px;color:#6b7280">
      <strong>${filtrados.length}</strong> sócio(s) registrado(s) na gestão de <strong>${pres}</strong>
    </div>
    <div style="max-height:400px;overflow-y:auto;border:1px solid #e5e7eb;border-radius:8px">
      <table style="width:100%;border-collapse:collapse;font-size:13px">
        <thead>
          <tr style="background:#f3f4f6;position:sticky;top:0">
            <th style="padding:8px 10px;text-align:left;border-bottom:1px solid #e5e7eb">Matrícula</th>
            <th style="padding:8px 10px;text-align:left;border-bottom:1px solid #e5e7eb">Nome</th>
            <th style="padding:8px 10px;text-align:left;border-bottom:1px solid #e5e7eb">Tipo</th>
            <th style="padding:8px 10px;text-align:left;border-bottom:1px solid #e5e7eb">Situação</th>
            <th style="padding:8px 10px;text-align:left;border-bottom:1px solid #e5e7eb">Data Cadastro</th>
          </tr>
        </thead>
        <tbody>
          ${filtrados.map(s => `
            <tr style="border-bottom:1px solid #f3f4f6;cursor:pointer" onclick="abrirFicha('${s.id}')" title="Clique para abrir a ficha">
              <td style="padding:8px 10px;font-weight:600">${s.matricula||'—'}</td>
              <td style="padding:8px 10px">${s.nome||'—'}</td>
              <td style="padding:8px 10px"><span class="tag tag-${getTipoNorm(s)}">${tipoLabel(s)}</span></td>
              <td style="padding:8px 10px"><span class="sit-badge ${getSitNorm(s)}">${getSitNorm(s)}</span></td>
              <td style="padding:8px 10px">${s.dataCadastro||'—'}</td>
            </tr>
          `).join('')}
        </tbody>
      </table>
...
function editarFicha(id){
  ...
  edit.innerHTML=`
    ...
    <div class="section"><div class="section-title">Situação e Dados do Sócio</div>
      <div class="grid2">
        ...
        <div class="field"><label>Tipo de Sócio</label><select id="e-tiposocio"><option value="contribuinte" ${getTipoNorm(s)==='contribuinte'?'selected':''}>Contribuinte</option><option value="proprietario" ${getTipoNorm(s)==='proprietario'?'selected':''}>Proprietário</option><option value="remido" ${getTipoNorm(s)==='remido'?'selected':''}>Remido</option><option value="contribuinte_policial" ${getTipoNorm(s)==='contribuinte_policial'?'selected':''}>Policial</option><option value="contribuinte_prefeitura" ${getTipoNorm(s)==='contribuinte_prefeitura'?'selected':''}>Prefeitura</option><option value="nao_socio" ${getTipoNorm(s)==='nao_socio'?'selected':''}>Não Sócio</option></select></div>
        <div class="field"><label>Data de Cadastro</label><input id="e-datacadastro" type="text" value="${s.dataCadastro||''}" placeholder="dd/mm/aaaa"/></div>
        <div class="field"><label>Origem do Título</label><select id="e-origemtitulo"><option value="" ${!s.origemTitulo?'selected':''}>—</option><option value="transferencia" ${s.origemTitulo==='transferencia'?'selected':''}>Transferência</option><option value="aquisicao" ${s.origemTitulo==='aquisicao'?'selected':''}>Aquisição</option><option value="heranca" ${s.origemTitulo==='heranca'?'selected':''}>Herança</option></select></div>
        <div class="field"><label>Data do Título</label><input id="e-datatitulo" type="date" value="${s.dataTitulo||''}"/></div>
        <div class="field"><label>Cedente</label><input id="e-cedente" value="${s.cedente||''}"/></div>
      </div>
    ...
    <div class="section" style="margin-top:16px"><div class="section-title">Dependentes</div>
      <div id="e-deps-list"></div>
      <button type="button" class="btn-back" onclick="addDepEdit()" style="margin-top:8px">+ Adicionar Dependente</button>
    </div>
  ...
  // Carrega dependentes existentes no editor
  setTimeout(() => {
    editDepsData = (s.dependentes||[]).map((d,i) => ({...d, id: i}));
...
let editDepsData = [], editDepCounter = 0;

function addDepEdit() {
  const i = editDepCounter++;
  editDepsData.push({id: i, nome: '', dataNasc: '', sexo: 'M', tipoDep: 'filho', foto: ''});
  renderEditDeps();
}

function removeDepEdit(id) {
  editDepsData = editDepsData.filter(d => d.id !== id);
  renderEditDeps();
}

function renderEditDeps() {
  const list = document.getElementById('e-deps-list');
  if (!list) return;
  if (!editDepsData.length) { list.innerHTML = '<p style="color:#9ca3af;font-size:13px">Nenhum dependente cadastrado.</p>'; return; }
  list.innerHTML = editDepsData.map((dep, i) => `
    <div class="dep-card">
      <div class="dep-header"><strong>Dependente #${i+1}</strong><button type="button" class="btn-del" onclick="removeDepEdit(${dep.id})">Remover</button></div>
      <div class="grid2">
...
async function carregarSocios() {
  try {
    ...
    if (response.ok) {
      ...
      // Mesclagem com planilha removida - dados vêm apenas do servidor
      try { localStorage.setItem(LOCAL_KEY, JSON.stringify(socios)); } catch(e) {}
      updateHeader();
      renderSocios();
      renderTitulos();
    ...
    // Servidor indisponível - usa apenas dados locais (sem fallback da planilha)
    updateHeader();
    renderSocios();
    renderTitulos();
    renderDashboard();
...
async function salvarEdicao(){
  ...
  if (novaMatricula && novaMatricula !== String(s.matricula)) {
    if (socios.find(x => String(x.id) !== String(fichaEditId) && String(x.matricula) === novaMatricula)) {
      const m=document.getElementById('ficha-msg');
      if(m){m.className='alert err';m.textContent='Matrícula já existe em outro sócio.';m.style.display='block';}
      return;
    }
    s.matricula = novaMatricula;
  }
  // numTitulo removido — usa-se a matrícula como identificador único
  s.tipoSocio    = document.getElementById('e-tiposocio')?.value || s.tipoSocio || 'contribuinte';
  s.dataCadastro = document.getElementById('e-datacadastro')?.value.trim() || s.dataCadastro || '';
  s.origemTitulo = document.getElementById('e-origemtitulo')?.value || s.origemTitulo || '';
  s.dataTitulo   = document.getElementById('e-datatitulo')?.value || s.dataTitulo || '';
  s.cedente      = document.getElementById('e-cedente')?.value.trim() || s.cedente || '';
