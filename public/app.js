...
function _filtrar(base){
  ...
  if(f.periodo && f.periodo!=='all'){
    ...
    if(r) arr = arr.filter(s => { const d=_parseDataCad(s.dataCadastro); return d && d>=r.ini && d<=r.fim; });
  }
  return arr;
}
...
function renderPiramide(elId, dataM, dataF, labels){
  ...
  const rowH = 30, gap=4, labW=70, barMax=180, midGap=20;
  const W = labW + barMax*2 + midGap*2;
  const H = labels.length * (rowH+gap) + 30;
  const rows = labels.map((l,i)=>{
    const y = i*(rowH+gap) + 10;
    const wM = (dataM[i]/max) * barMax;
    const wF = (dataF[i]/max) * barMax;
    const xM = labW + barMax - wM + midGap;
    const xF = labW + barMax + midGap*2;
    return `
      <text x="${labW + barMax + midGap}" y="${y+rowH/2+4}" text-anchor="middle" font-size="11" fill="#374151" font-weight="700">${l}</text>
      <rect x="${xM}" y="${y}" width="${wM}" height="${rowH}" fill="#1B3A70" rx="3"><title>Masculino ${l}: ${dataM[i]}</title></rect>
      <text x="${xM-4}" y="${y+rowH/2+4}" text-anchor="end" font-size="11" fill="#374151" font-weight="700">${dataM[i]||''}</text>
      <rect x="${xF}" y="${y}" width="${wF}" height="${rowH}" fill="#ec4899" rx="3"><title>Feminino ${l}: ${dataF[i]}</title></rect>
      <text x="${xF+wF+4}" y="${y+rowH/2+4}" font-size="11" fill="#374151" font-weight="700">${dataF[i]||''}</text>
    `;
  }).join('');
  el.innerHTML = `<svg width="100%" viewBox="0 0 ${W} ${H}" preserveAspectRatio="xMidYMid meet">
    <text x="${labW+barMax/2 - midGap/2}" y="8" text-anchor="middle" font-size="11" fill="#1B3A70" font-weight="700">MASCULINO</text>
    <text x="${labW+barMax + midGap*2 + barMax/2}" y="8" text-anchor="middle" font-size="11" fill="#ec4899" font-weight="700">FEMININO</text>
    ${rows}
  </svg>`;
}
...
function renderDashboard(){
  ...
  // Evolução por ano
  const porAno = {};
  arr.forEach(s=>{ const d=_parseDataCad(s.dataCadastro); if(d){ porAno[d.getFullYear()]=(porAno[d.getFullYear()]||0)+1; }});
  ...
  const sparkVals = meses12.map(mm => arr.filter(s=>{const d=_parseDataCad(s.dataCadastro);return d && d.getFullYear()===mm.y && d.getMonth()===mm.m;}).length);
  ...
  function _trend(metricFn){
    ...
    const inRange = (s,r) => { const d=_parseDataCad(s.dataCadastro); return d && d>=r.ini && d<=r.fim; };
...
function renderComparativo(){
  ...
  const calc = (nome) => {
    ...
    const anos = list.map(s=>{const d=_parseDataCad(s.dataCadastro);return d?d.getFullYear():null;}).filter(y=>y);
