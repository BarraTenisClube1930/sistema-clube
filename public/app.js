...
function renderDashboard(){
  ...
  if(total===0){
    ...
    ['dash-alertas','dash-insight','kpi-row','chart-donut','chart-spark','chart-tipo','chart-sit','chart-piramide','chart-sexo','chart-estcivil','chart-bairro-heat','chart-ativ','chart-area','tabela-presidentes','comparativo-out','lista-resolver','lista-aniver','lista-recentes','chart-completude','chart-deps','score-num','score-detail']
  ...
  arr.forEach(s=>{
    ...
    if(!presStats[p]) presStats[p] = {nome:p, total:0, props:0, remidos:0};
    presStats[p].total++;
    if(getTipoNorm(s)==='proprietario') presStats[p].props++;
    if(getTipoNorm(s)==='remido')       presStats[p].remidos++;
  });
  ...
  _bar('chart-deps',[{l:'Com dependentes',v:comDeps},{l:'Sem dependentes',v:total-comDeps}]);
}
...
function renderTabelaPresidentes(stats){
  ...
  el.innerHTML = `<div style="overflow-x:auto"><table class="pres-table">
    ...
    <tbody>${stats.map(p=>`<tr>
      <td><span class="pres-name" onclick="mostrarSociosPorPresidente('${escAttr(p.nome)}',null)">${sanitize(p.nome)}</span></td>
...
function renderComparativo(){
  ...
  const calc = (nome) => {
    const list = socios.filter(s=>s.presidente===nome);
    ...
    return { total:list.length, props, remidos, ativos, idadeMedia, mediaAno:(list.length/span).toFixed(1), periodo: anos.length?`${Math.min(...anos)}–${Math.max(...anos)}`:'—' };
  };
...
function abrirModalCarteirinhasLote() {
  ...
  const gestoes = Array.from(new Set((socios||[]).map(s => (s.presidente||'').trim()).filter(Boolean))).sort();
