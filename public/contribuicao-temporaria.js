...
(function(){
  ...
  // ---------- helpers ----------
  // socios/fichaEditId são declarados com `let` em app.js (script-scope, NÃO viram window.*)
  // Usamos esses getters para ler de qualquer escopo de forma segura.
  function _socios(){
    try { if(typeof socios !== 'undefined' && Array.isArray(socios)) return socios; } catch(e){}
    var w = (typeof window!=='undefined') ? window['socios'] : null;
    return Array.isArray(w) ? w : [];
  }
  function _fichaEditId(){
    try { if(typeof fichaEditId !== 'undefined') return fichaEditId; } catch(e){}
    return (typeof window!=='undefined') ? window['fichaEditId'] : null;
  }
  ...
  function wrapSalvarEdicao(){
    ...
    const wrapped = async function(){
      try{
        const id = _fichaEditId();
        const s  = _socios().find(x=>String(x.id)===String(id));
  ...
  function renderDashboardCard(){
    ...
    const props = _socios().filter(s=>isPropTipo(s.tipoSocio||s.tipo));
