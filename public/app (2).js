...
function _coletarPessoasLote() {
  // v5: nunca incluir sócios irregulares no lote
...
async function gerarPdfCarteirinhasLote() {
  ...
  let pessoas = _coletarPessoasLote();
  // v5: bloquear sócios irregulares
  const antes = pessoas.length;
  pessoas = pessoas.filter(p => {
    const sx = socios.find(x => String(x.matricula) === String(p.matricula));
    return !sx || !isIrregular(sx);
  });
  const removidos = antes - pessoas.length;
  if (!pessoas.length) { alert('Nenhuma carteirinha atende aos filtros' + (removidos ? ' (' + removidos + ' sócio(s) irregular(es) excluído(s) automaticamente).' : '.')); return; }
  if (removidos > 0) {
    if (!confirm(removidos + ' sócio(s) com situação IRREGULAR foram excluídos automaticamente do lote (não geram carteirinha).\n\nContinuar com ' + pessoas.length + ' carteirinha(s)?')) return;
  }
